...
## 编译
### 安装编译环境
    sudo yum -y update && sudo yum install -y epel-release wget cmake3 readline-devel.x86_64 gcc gcc-g++ tree python3 python3-devel
...
## 准备安装文件
### 下载部署依赖包
    mkdir -p /home/top && cd /home/top && cp deploy_allinone.tar.gz /home/top/
...
#### 节点注册
    ...
    注册成功后查询的预期结果如下：
    The miner has not joined in node-standby-pool...
    ...
    "account_addr" : "T00000XXXXXXXX",
    ...
    "node_deposit" : 200000000000,
    ...
    "node_sign_key" : "BNi...nEg=",
    ...
    "registered_node_type" : "edge",
    ...
#### 启动节点
    输入以下命令根据已注册的账号启动节点进程：
    cd /home/top && sh start_node_allinone.sh -t edge -a T00000XXXXXXXX -i 127.0.0.7 -p 9007
    ***输入 sh start_node_allinone.sh -h 可以查看帮助信息***
...
## 确认环境[以6个SEED创世进行介绍]：
    ...
    内核版本：Linux version 3.10.0-1127.13.1.el7.x86_64
    用户：root/123456
    IP段：192.168.50.135 ~ 192.168.50.142
    主机规划：
        192.168.50.135 作为ansible操作主机
        192.168.50.136 ~ 192.168.50.141 作为部署rec节点的主机
        192.168.50.142 作为新入网节点的主机
...
### 安装依赖
    ...
    sudo yum -y update && sudo yum install -y epel-release ansible jq tree python3 python3-devel
### 修改ssh连接配置
    输入以下命令禁用ssh指纹校验
    echo "StrictHostKeyChecking no" >> /etc/ssh/ssh_config
### 下载部署依赖包
    创建部署文件夹并下载部署依赖包
    mkdir -p /home/top && && cp deploy_multi_hosts.tar.gz /home/top/
...
### 检查安装依赖文件
    ...
    ***
    ...
    .edge_config.json是topio的配置文件
    keystore文件夹内包含3个创世账号、3个TCC账号、6个基金会节点账号的私钥文件
    deploy_multi.sh是部署组网的脚本
    ***
## 部署组网
    root登录192.168.50.135主机，执行以下命令部署：
    cd /home/top && sh deploy_multi.sh -j /home/top/topo.json -x /home/top/xtopchain
## 查询创世结果
...
### 修改topio配置文件
    ...
    sed -i "s#127.0.0.1#192.168.50.136#g" /home/top/.edge_config.json
### topio加载创世账号
    参考：部署AllInOne - 使用链上业务功能 - topio加载创世账号
### 创建新账号
    参考：部署AllInOne - 使用链上业务功能 - 创建新账号
### 节点注册和启动挖矿
#### 节点注册
    参考：部署AllInOne - 使用链上业务功能 - 节点注册和启动挖矿 - 节点注册
#### 启动网内新节点
    输入以下命令根据已注册的账号启动节点进程：
    cd /home/top && sh start_node_multi.sh -j /home/top/topo.json -x /home/top/xtopchain -t edge -a T00000XXXXXXXX -i 192.168.50.142
    ***输入 sh start_node_multi.sh -h 可以查看帮助信息***
