...
TEST_F(test_eth_root, transfer_three_tx) {
    ...
    header_para.m_gaslimit = (uint64_t)jsToU256("0x7a1200");
    header_para.m_baseprice = jsToU256("0x7");
    ...
    ASSERT_EQ(ethheader.get_gasused(), (uint64_t)jsToU256("0xf618"));
    ASSERT_EQ(ethheader.get_baseprice(), jsToU256("0x7"));
...
TEST_F(test_eth_root, contract_deploy_one_tx) {
    {
        ...
        xeth_receipt_t receipt1;
        receipt1.set_tx_status(ethreceipt_status_successful);
        receipt1.set_cumulative_gas_used((uint64_t)jsToU256("0x702fa"));
        receipts.push_back(receipt1);
    ...
    }
    {
        ...
        tx.set_max_fee_per_gas(jsToU256("0x9502f90e"));
        tx.set_gas(jsToU256("0x702fa"));
        tx.set_to(common::xeth_address_t::zero());
        tx.set_value(jsToU256("0x0"));
...
TEST_F(test_eth_root, contract_call_one_tx_1) {
    {
        ...
        xeth_receipt_t receipt1;
        receipt1.set_tx_status(ethreceipt_status_successful);
        receipt1.set_cumulative_gas_used((uint64_t)jsToU256("0xaad8"));
        receipts.push_back(receipt1);
...

{
    xeth_receipt_t receipt1;
    receipt1.set_tx_status(ethreceipt_status_successful);
    receipt1.set_cumulative_gas_used((uint64_t)jsToU256("0x57506"));
...
}
{
    ...
    ASSERT_EQ(ethheader.get_gaslimit(), header_para.m_gaslimit);
    ASSERT_EQ(ethheader.get_gasused(), (uint64_t)jsToU256("0x57506"));
    ASSERT_EQ(ethheader.get_baseprice(), jsToU256("0x7"));
    ASSERT_EQ(ethheader.get_coinbase().to_hex_string(), "0x0000000000000000000000000000000000000000");
}
