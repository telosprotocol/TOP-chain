...
class xdatamock_table {
 ...
 public:
    explicit xdatamock_table(uint16_t tableid = 1, uint32_t user_count = 4, xstore_face_t* store = nullptr) {
    ...
    void    set_xstore(xstore_face_t* store) {m_store = store;}
    ...
    xblock_ptr_t generate_tableblock() {
        ...
        if (prev_tableblock->get_height() > (prev_tableblock->get_last_full_block_height() + enum_default_full_table_interval_count)) {
    ...
    void on_table_finish(const xblock_ptr_t & block) {
        ...
        if (block->is_lighttable()) {
            ...
            xassert(m_table_blockchain->add_full_table(block.get(), m_last_full_mbt));
            m_last_full_mbt = m_table_blockchain->get_table_mbt();  // save new full mbt
        }
