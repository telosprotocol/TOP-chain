...
class xdatamock_table : public base::xvaccount_t {
 ...
 private:
    ...
    xblock_ptr_t generate_batch_table(const data::xblock_consensus_para_t & cs_para) {
        ...
        xassert(ret);
        std::error_code ec;
        auto table_mpt = state_mpt::xtop_state_mpt::create(common::xaccount_address_t{get_cert_block()->get_account()}, top::xhash256_t(last_state_root.to_bytes()), m_store.get(), state_mpt::xstate_mpt_cache_t::instance(), ec);
        if (ec) {
            xassert(false);
        }
