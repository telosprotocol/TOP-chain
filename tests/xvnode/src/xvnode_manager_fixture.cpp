...
std::shared_ptr<top::vnode::xvnode_face_t> xvnode_factory::create_vnode_at(std::shared_ptr<election::cache::xgroup_element_t> const & group_element) const {
    ...
    case top::common::xnode_type_t::full_archive:
        return std::make_shared<xdummy_vnode<top::common::xnode_type_t::full_archive>>(std::move(address));
...
xvnode_manager_fixture::xvnode_manager_fixture()
  : top::vnode::xvnode_manager_t{top::make_observer(&xdummy_chain_timer), top::make_observer(&xdummy_vhost), top::make_unique<xvnode_factory>()}
