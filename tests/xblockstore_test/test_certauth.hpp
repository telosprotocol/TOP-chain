...
private:
    ...
    test_certauth(size_t node_count) : m_node_count(node_count) {
        ...
        for(size_t i = 0; i < node_count; ++i) {
            ...
            xdbg("test_certauth::test_certauth,nodes for signer(%" PRIx64 " : %" PRIx64 " )",node_addr.high_addr,node_addr.low_addr);
...
public:
    static test_certauth& instance(size_t node_count = 4)
    {
        static test_certauth xca(node_count);
        return xca;
    }
    ...
    void do_validate(base::xvblock_t* block) {
        xassert(block != nullptr);
        std::random_device rd;
        std::uniform_int_distribution<size_t> dist(0, m_node_count - 1);
        xvip2_t node = m_consensus_nodes[dist(rd)]->get_xip2_addr();
