...
TEST_F(test_block_notify, block_event_normal_block) {
    ...
    for (uint64_t i = 1; i <= count; i++) {
        ...
        curr_block = test_blocktuil::create_next_emptyblock(prev_block);
        ...
        if (curr_block->get_height() >= 3) {
            uint64_t chainheight = store->get_blockchain_height(address);
            ASSERT_EQ(chainheight, curr_block->get_height() - 2);
        }
    ...
    ASSERT_EQ(count - 2, notified_count);
}
