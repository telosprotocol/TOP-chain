...
TEST_F(test_new_receipt_queue, receipt_queue_basic) {
    ...
    for (uint32_t i = 0; i < tx_num; i++) {
        ...
        ASSERT_EQ(ret, 0);
        auto find_receipt = receipt_queue.find(recv_txs[i]->get_tx_hash());
    ...
    auto receipts1 = receipt_queue.get_txs(10, 10, receiptid_state, id_height_map, confirm_txs_num);
    ASSERT_EQ(receipts1.size(), tx_num);

    xreceiptid_pair_t receiptid_pair(1, 0, 1, 0, 0);
    ...
    auto receipts2 = receipt_queue.get_txs(10, 10, receiptid_state, id_height_map, confirm_txs_num);

    ASSERT_EQ(receipts2.size(), tx_num - 1);

    auto find_receipt0 = receipt_queue.find(recv_txs[0]->get_tx_hash());
    ...
    auto find_receipt = receipt_queue.find(recv_txs[1]->get_tx_hash());
