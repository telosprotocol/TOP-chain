...
TEST_F(test_tablemaker, version_1) {
...

    {
        ...
        EXPECT_EQ(proposal_block->get_block_version(), base::enum_xvblock_version_1);
        ...
        auto headers = proposal_block->get_unit_headers();
        EXPECT_EQ(headers.size(), 1);
        for (auto & header : headers) {
            EXPECT_EQ(header->get_extra_data().empty(), true);
            EXPECT_EQ(header->get_block_version(), base::enum_xvblock_version_1);
        }
    }
}
...
TEST_F(test_tablemaker, version_2) {
...

    {
        ...
        auto txs = proposal_block->get_txs();
        EXPECT_EQ(txs.size(), tx_cnt);
        EXPECT_EQ(proposal_block->get_block_version(), base::enum_xvblock_version_2);
        ...
        for (auto & header : headers) {
            ...
            EXPECT_EQ(header->get_block_version(), base::enum_xvblock_version_2);
        }
    }
}
