...
TEST_F(test_suite_zec_reward_contract_t, preprocess_workload) {
    ...
    map_nodes.clear();
    {
        ...
        node.m_account = ss.str();
        node.m_account_mortgage = 0;
        node.miner_type(common::xminer_type_t::consensus);
        node.m_vote_amount = 900;
        map_nodes[ss.str()] = node;
    }
...
TEST_F(test_suite_zec_reward_contract_t, test_issue_model) {
    ...
    for (auto i = 0; i < 256; i++) {
        ...
        for (auto j = 0; j < 2; j++) {
            ...
            int ret = registerNode(node.m_account, node.m_account_mortgage, common::to_string(node.m_registered_miner_type));
            ASSERT_TRUE(ret == 0);
    ...
    auto print_rewards = []() {
        ...
        for (auto i = 0; i < 256; i++) {
            ...
            for (auto m : node_rewards) {
                ...
                std::stringstream ss;
                ss  << "[test_issue_model] "
                    << "node_account: " << m.first << ", " << "node_type: " << common::to_string(reg_node_info.m_registered_miner_type) << ", "
