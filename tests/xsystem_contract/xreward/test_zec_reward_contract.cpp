...
TEST_F(xtest_reward_contract_t, test_accounts_vec) {
    ...
    for (size_t i = 0; i < accounts_vec.size(); i++) {
        uint32_t table_id = 0;
        EXTRACT_TABLE_ID(common::xaccount_address_t{accounts_vec[i]}, table_id);
        EXPECT_EQ(table_id, i / 64);
    }
}
...
TEST_F(xtest_reward_contract_t, test_task_deserialize) {
    xreward_dispatch_task task1;
    task1.action = "action";
    task1.contract = "contract";
    task1.params = "params";
    task1.onchain_timer_round = 100;
    auto str = reward_task_serialize(task1);
    {
        xreward_dispatch_task task2;
        base::xstream_t stream{base::xcontext_t::instance(), (uint8_t *)str.c_str(), (uint32_t)str.size()};
        task2.serialize_from(stream);
        EXPECT_EQ(task2.action, "action");
        EXPECT_EQ(task2.contract, "contract");
        EXPECT_EQ(task2.params, "params");
        EXPECT_EQ(task2.onchain_timer_round, 100);
    }
    auto task3 = reward_task_deserialize(str);
    {
        base::xstream_t stream{base::xcontext_t::instance()};
        task3.serialize_to(stream);
        EXPECT_EQ(std::string((char *)stream.data(), stream.size()), str);
    }
    EXPECT_EQ(task3.action, "action");
    EXPECT_EQ(task3.contract, "contract");
    EXPECT_EQ(task3.params, "params");
    EXPECT_EQ(task3.onchain_timer_round, 100);
}
...
TEST_F(xtest_reward_contract_t, test_get_task_id_1) {
    ...
    auto unitstate = std::make_shared<xunit_bstate_t>(vbstate.get());
    auto account_context = std::make_shared<xaccount_context_t>(unitstate);
    auto contract_helper = std::make_shared<xcontract_helper>(account_context.get(), common::xnode_id_t{sys_contract_zec_reward_addr}, sys_contract_zec_reward_addr);
    set_contract_helper(contract_helper);
...
TEST_F(xtest_reward_contract_t, test_get_task_id_2) {
    ...
    auto unitstate = std::make_shared<xunit_bstate_t>(vbstate.get());
    auto account_context = std::make_shared<xaccount_context_t>(unitstate);
    auto contract_helper = std::make_shared<xcontract_helper>(account_context.get(), common::xnode_id_t{sys_contract_zec_workload_addr}, sys_contract_zec_reward_addr);
    set_contract_helper(contract_helper);
    MAP_CREATE(XPORPERTY_CONTRACT_TASK_KEY); 

    std::map<std::string, std::string> tasks_map_str;
    for (auto i = 0; i < 100; i++) {
        std::string task_str = reward_task_serialize(xreward_dispatch_task{});
        MAP_SET(XPORPERTY_CONTRACT_TASK_KEY, base::xstring_utl::tostring(i), task_str);
    }

    EXPECT_EQ(get_task_id(), 100);
}

TEST_F(xtest_reward_contract_t, test_get_task_id_internal_1) {
    std::map<std::string, std::string> tasks_map_str;
    EXPECT_EQ(get_task_id_internal(tasks_map_str), 0);
}

TEST_F(xtest_reward_contract_t, test_get_task_id_internal_2) {
    std::map<std::string, std::string> tasks_map_str;
    for (auto i = 0; i < 100; i++) {
        std::string task_str = reward_task_serialize(xreward_dispatch_task{});
        tasks_map_str.insert(std::make_pair(base::xstring_utl::tostring(i), task_str));
    }
    EXPECT_EQ(get_task_id_internal(tasks_map_str), 100);
}
...
TEST_F(xtest_reward_contract_t, test_add_task_1) {
    ...
    EXPECT_EQ(tasks_map_str.size(), 100);
    for (auto i = 0; i < 100; i++) {
        std::stringstream ss;
...
TEST_F(xtest_reward_contract_t, test_add_task_internal_1) {
    for (auto i = 0; i < 100; i++) {
        auto pair = add_task_internal(i,
                                      i * 100,
                                      std::string{"contract"} + base::xstring_utl::tostring(i),
                                      std::string{"action"} + base::xstring_utl::tostring(i),
                                      std::string{"param"} + base::xstring_utl::tostring(i));
        EXPECT_EQ(base::xstring_utl::touint64(pair.first), i);
        xreward_dispatch_task task = reward_task_deserialize(pair.second);
        EXPECT_EQ(task.onchain_timer_round, i * 100);
        EXPECT_EQ(task.contract, std::string{"contract"} + base::xstring_utl::tostring(i));
        EXPECT_EQ(task.action, std::string{"action"} + base::xstring_utl::tostring(i));
        EXPECT_EQ(task.params, std::string{"param"} + base::xstring_utl::tostring(i));
...
TEST_F(xtest_reward_contract_t, test_execute_task_internal) {
    auto exe_addr = std::string{sys_contract_zec_reward_addr};
    auto vbstate = make_object_ptr<xvbstate_t>(sys_contract_zec_reward_addr, 1 , 1, std::string{}, std::string{}, 0, 0, 0);
    auto unitstate = std::make_shared<xunit_bstate_t>(vbstate.get());
    auto account_context = std::make_shared<xaccount_context_t>(unitstate);
    ...
    top::xstake::uint128_t community_reward{1500000};
    ...
    for (auto i = 0; i < 64; i++) {
        ...
        std::map<common::xaccount_address_t, top::xstake::uint128_t> node_reward;
        for (auto j = 0; j < 10; j++) {
        ...
        if (i < 32) {
            std::map<common::xaccount_address_t, top::xstake::uint128_t> node_dividend;
            for (auto j = 0; j < 3; j++) {
                std::string node_addr = "node" + base::xstring_utl::tostring(j);
    ...
    std::map<std::string, std::string> tasks_map_str_temp = tasks_map_str;
    std::map<std::string, uint64_t> transfer_map;
    std::vector<xreward_dispatch_task> call_vec;
    std::vector<std::string> rm_vec;
    execute_task_internal(tasks_map_str, transfer_map, call_vec, rm_vec);
    EXPECT_EQ(transfer_map.size(), 15);
    EXPECT_EQ(call_vec.size(), 1);
    EXPECT_EQ(rm_vec.size(), 16);
    for (size_t i = 0; i < 16; i++) {
        EXPECT_EQ(tasks_map_str_temp.count(rm_vec[i]), true);
    }
}
...
TEST_F(xtest_reward_contract_t, test_nodes_rewards) {
    ...
    std::map<common::xaccount_address_t, std::map<common::xaccount_address_t, top::xstake::uint128_t>> table_node_reward_detail;
    std::map<common::xaccount_address_t, std::map<common::xaccount_address_t, top::xstake::uint128_t>> table_node_dividend_detail;
    calc_nodes_rewards_v5(TIMER_BLOCK_HEIGHT_PER_YEAR + 10 + 1, 1, onchain_param, property_param, issue_detail, node_reward_detail, node_dividend_detail, community_reward);
...
TEST_F(xtest_reward_contract_t, test_dispatch_all_reward_1) {
    ...
    for (auto i = task_id; i < task_id + 64; i++) {
        ...
        EXPECT_EQ(task.onchain_timer_round, 100);
        base::xstream_t stream{base::xcontext_t::instance(), (uint8_t *)task.params.c_str(), (uint32_t)task.params.size()};
        std::map<std::string, top::xstake::uint128_t> issuances;
        uint64_t time = 0;
        stream >> time;
        stream >> issuances;
        EXPECT_EQ(time, 100);
...
TEST_F(xtest_reward_contract_t, test_calc_rewards_years) {
    ...
    // top::xstake::uint128_t total = static_cast<top::xstake::uint128_t>(TOTAL_RESERVE) * REWARD_PRECISION;
    top::xstake::uint128_t year1_total = static_cast<top::xstake::uint128_t>(TOTAL_RESERVE) * REWARD_PRECISION * additional_issue_year_ratio / 100;
    top::xstake::uint128_t year2_total = (static_cast<top::xstake::uint128_t>(TOTAL_RESERVE) * REWARD_PRECISION - year1_total) * additional_issue_year_ratio / 100;
    // case 0: time 0
    top::xstake::uint128_t issue1 = calc_total_issuance(TIMER_BLOCK_HEIGHT_PER_YEAR, min_ratio_annual_total_reward, additional_issue_year_ratio, record);
    top::xstake::uint128_t issue2 = calc_total_issuance(TIMER_BLOCK_HEIGHT_PER_YEAR * 2, min_ratio_annual_total_reward, additional_issue_year_ratio, record);
    top::xstake::uint128_t issue3 = calc_total_issuance(TIMER_BLOCK_HEIGHT_PER_YEAR * 3, min_ratio_annual_total_reward, additional_issue_year_ratio, record);
    top::xstake::uint128_t issue4 = calc_total_issuance(TIMER_BLOCK_HEIGHT_PER_YEAR * 4, min_ratio_annual_total_reward, additional_issue_year_ratio, record);
    top::xstake::uint128_t issue5 = calc_total_issuance(TIMER_BLOCK_HEIGHT_PER_YEAR * 5, min_ratio_annual_total_reward, additional_issue_year_ratio, record);
