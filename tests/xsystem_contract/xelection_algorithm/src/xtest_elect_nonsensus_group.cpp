...
TEST_F(xtest_elect_nonconsensus_group_contract_fixture_t, test_edge) {
    ...
    ASSERT_FALSE(m_elect_nonconsensus_group.test_elect(zid, cid, gid, 0, 0, group_size_range, standby_network_result, election_network_result));
    ...
    ASSERT_TRUE(m_elect_nonconsensus_group.test_elect(zid, cid, gid, 0, 0, group_size_range, standby_network_result, election_network_result));
    ...
    ASSERT_FALSE(m_elect_nonconsensus_group.test_elect(zid, cid, gid, 1, 1, group_size_range, standby_network_result, election_network_result));
    ASSERT_EQ(election_result.size(), 10);
    ASSERT_EQ(election_result.start_time(), common::xlogic_time_t{0});
    ASSERT_EQ(election_result.timestamp(), common::xlogic_time_t{0});
    ...
    ASSERT_TRUE(m_elect_nonconsensus_group.test_elect(zid, cid, gid, 2, 2, group_size_range, standby_network_result, election_network_result));
...
TEST_F(xtest_elect_nonconsensus_group_contract_fixture_t, test_archive) {
    common::xnode_type_t node_type{common::xnode_type_t::archive};
