...
TEST_F(xtest_elect_nonconsensus_group_contract_fixture_t, test_edge) {
    ...
    ASSERT_EQ(zec_standby_result.size(), 0);
    auto simple_standby_result = data::standby::select_standby_nodes(zec_standby_result, node_type);
    ASSERT_FALSE(m_elect_nonconsensus_group.test_elect(zid, cid, gid, 0, 0, group_size_range, simple_standby_result, election_network_result));
    ...
    ASSERT_TRUE(m_elect_nonconsensus_group.test_elect(zid, cid, gid, 0, 0, group_size_range, simple_standby_result, election_network_result));
    ...
    simple_standby_result = data::standby::select_standby_nodes(zec_standby_result, node_type);
    ASSERT_FALSE(m_elect_nonconsensus_group.test_elect(zid, cid, gid, 1, 1, group_size_range, simple_standby_result, election_network_result));
    ASSERT_EQ(election_result.size(), 10);
    ASSERT_EQ(election_result.start_time(), common::xlogic_time_t{0});
    ASSERT_EQ(election_result.timestamp(), common::xlogic_time_t{0});

    dereg_nodes_from_standby(10, TEST_NODE_ID_PREFIX);
    add_nodes_to_standby(600, node_type, TEST_NODE_ID_PREFIX);
    ASSERT_EQ(zec_standby_result.size(), 600);
    simple_standby_result = data::standby::select_standby_nodes(zec_standby_result, node_type);
    ASSERT_TRUE(m_elect_nonconsensus_group.test_elect(zid, cid, gid, 2, 2, group_size_range, simple_standby_result, election_network_result));
    ASSERT_EQ(election_result.size(), XGET_ONCHAIN_GOVERNANCE_PARAMETER(max_edge_group_size));
}
