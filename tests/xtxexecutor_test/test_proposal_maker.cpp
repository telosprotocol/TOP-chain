...
TEST_F(test_proposal_maker, table_maker_1) {
    ...
    std::shared_ptr<xunit_service::xblock_maker_face> blockmaker = xblockmaker_factory::create_txpool_block_maker(make_observer(resouces->get_store()), resouces->get_blockstore(), resouces->get_txpool());
...
TEST_F(test_proposal_maker, table_maker_2) {
    ...
    auto     xtxpool = xtxpool_instance::create_xtxpool_inst(make_observer(store_ptr), blockstore, make_observer(mbus.get()), nullptr);
...
TEST_F(test_proposal_maker, table_maker_3) {
    ...
    xblock_maker_para_t proposal_para(new_clock, view_id);
    ...
    ASSERT_EQ(true, blockmaker->update_txpool_txs(proposal_para, timestamp, table_para));
