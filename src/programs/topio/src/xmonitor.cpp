...
bool xmonitor_t::monitor_cpu_and_network() {
    ...
    if (0 == m_last_scan_timestamp)  // first time
    {
        ...
        xsys_utl::get_sys_net_info(
            default_eth_device, m_last_sys_rx_bytes, m_last_sys_rx_packets, m_last_sys_rx_drop_packets, m_last_sys_tx_bytes, m_last_sys_tx_packets, m_last_sys_tx_drop_packets);
    } else {
        ...
        if (duration_ms > xcontext_t::enum_system_scan_interval) {
            ...
            if (xsys_utl::get_sys_net_info(
                    default_eth_device, now_sys_rx_bytes, now_sys_rx_packets, now_sys_rx_drop_packets, now_sys_tx_bytes, now_sys_tx_packets, now_sys_tx_drop_packets)) {
...
void xmonitor_t::dump_net() {
    TOP_INFO("%5s | %10s %12s %8s %11s %13s", default_eth_device.c_str(), "speed/kbps", "throuout/pps", "drop/pps", "bytes/month", "packets/month");
    TOP_INFO("%5s | %10ld %12ld %8ld %11ld %13ld",
             "in",
             (long)m_sys_net_in_speed,
             (long)m_sys_net_in_throughout,
             (long)m_sys_net_in_drop,
             (long)m_sys_net_in_bytes_of_month,
             (long)m_sys_net_in_packets_of_month);
    TOP_INFO("%5s | %10ld %12ld %8ld %11ld %13ld",
             "out",
             (long)m_sys_net_out_speed,
             (long)m_sys_net_out_throughout,
             (long)m_sys_net_out_drop,
             (long)m_sys_net_out_bytes_of_month,
             (long)m_sys_net_out_packets_of_month);
}
