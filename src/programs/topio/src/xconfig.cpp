...
bool xconfig::save(const std::string & config_file, std::unordered_map<std::string, std::string> & map) {
    xJson::Value root;
    for (auto & enty : map) {
...
void xconfig::set_option_param(bool & destination, const std::string & item, const std::string & sub_item) {
    if (!m_root[item].empty() && !m_root[item][sub_item].empty() && m_root[item][sub_item].isBool()) {
        destination = m_root[item][sub_item].asBool();
    }
}

void xconfig::set_option_param(std::string & destination, const std::string & item, const std::string & sub_item) {
    if (!m_root[item].empty() && !m_root[item][sub_item].empty() && m_root[item][sub_item].isString()) {
        destination = m_root[item][sub_item].asString();
    }
}

void xconfig::set_option_param(uint32_t & destination, const std::string & item, const std::string & sub_item) {
    if (!m_root[item].empty() && !m_root[item][sub_item].empty() && m_root[item][sub_item].isUInt()) {
        destination = m_root[item][sub_item].asUInt();
    }
}

void xconfig::set_option_param(uint64_t & destination, const std::string & item, const std::string & sub_item) {
    if (!m_root[item].empty() && !m_root[item][sub_item].empty() && m_root[item][sub_item].isUInt64()) {
        destination = m_root[item][sub_item].asUInt64();
    }
}

void xconfig::set_option_param(uint16_t & destination, const std::string & item, const std::string & sub_item) {
    if (!m_root[item].empty() && !m_root[item][sub_item].empty() && m_root[item][sub_item].isUInt()) {
        destination = m_root[item][sub_item].asUInt();
    }
}

void xconfig::set_option_param(int32_t & destination, const std::string & item, const std::string & sub_item) {
