...
int main(int argc, char * argv[]) {
    ...
    uint16_t const ledger_id = top::base::xvaccount_t::make_ledger_id(static_cast<top::base::enum_xchain_id>(network_id.value()), zone_index);
    for (int32_t i = 0; i < count; i++) {
        switch (account_address_type) {
        case top::base::enum_vaccount_addr_type_secp256k1_eth_user_account:
            XATTRIBUTE_FALLTHROUGH;
        case top::base::enum_vaccount_addr_type_secp256k1_evm_user_account: {
            auto hash = top::utl::xsha2_512_t::digest(std::to_string(std::time(nullptr) + i));
            auto prefix_account = top::common::xaccount_address_t::build_from(top::utl::xcrypto_util::make_address_by_assigned_key(hash.data(), account_address_type, ledger_id));
            std::cout << prefix_account.to_string() << "\ttable id: " << prefix_account.table_id().value() << std::endl;
            break;
        }

        case top::base::enum_vaccount_addr_type_secp256k1_user_account: {
