...
common::xenum_node_type xtop_sync_object::get_table_type(const std::string& table_prefix) const {
    ...
    } else if (table_prefix == sys_contract_eth_table_block_addr) {
        return common::xnode_type_t::evm;
    } else if (table_prefix == sys_contract_relay_table_block_base_addr) {
        return common::xnode_type_t::relay;
    }
    xdbg("unknown table type: %s", table_prefix.c_str());
...
std::string xtop_sync_object::status() const {
    ...
    for (int32_t i = enum_chain_sync_policy_checkpoint; i >= 0; i--) {
        ...
        std::map<common::xenum_node_type, std::map<uint32_t, xsync_progress_t>> tables_progress;
        std::map<common::xenum_node_type, xsync_table_data> table_display;
        ...
        for (const auto &role_it: roles) {
            ...
            for (const auto &it: chains) {
                ...
                tables_progress[get_table_type(table_prefix)].insert(std::make_pair(table_id, info));
                table_display[get_table_type(table_prefix)].total_cur_height += info.cur_height;
                table_display[get_table_type(table_prefix)].total_max_height += info.max_height;
            }
        }
