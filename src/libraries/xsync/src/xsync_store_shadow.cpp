...
void xsync_chain_spans_t::initialize() {
    ...
    while (1) {
        auto vbindex = m_store->load_block_object(m_account, m_connect_to_genesis_height + 1);
        if (vbindex == nullptr)
            break;
    ...
    xinfo("xsync_store_shadow_t::initialize after, account:%s, new height=%llu, updated height=%llu, old = %llu",
