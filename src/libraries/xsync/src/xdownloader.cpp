...
void xaccount_timer_t::set_chain(xchain_downloader_face_ptr_t &chain_downloader) {
    auto chain = m_chains.find(chain_downloader->get_address());
    std::string a;
    if (chain == m_chains.end()) {
        m_chains.insert(std::make_pair(chain_downloader->get_address(), chain_downloader));
    }
}
