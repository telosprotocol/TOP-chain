...
#include "xpara_proxy/xcons_mgr_builder.h"
#include "xpara_proxy/xbeacon_chain_para_proxy.h"
...
xtop_para_proxy::xtop_para_proxy(common::xnode_id_t const & node_id, xpublic_key_t const & public_key, std::string const & sign_key)
  : m_node_id{node_id}
...
void xtop_para_proxy::start() {
    if ((m_store == nullptr) || !m_store->open()) {
        xwarn("xtop_para_proxy::start db open failed!");
        exit(0);
    }
    ...
    m_chain_para_proxys.push_back(
        top::make_unique<xbeacon_chain_para_proxy_t>(make_observer(this), m_blockstore, m_nodesvr_ptr, m_cert_ptr, make_observer(m_grpc_thread), make_observer(m_sync_thread), sync_account_thread_pool, sync_handler_thread_pool));
    ...
    if (m_elect_main->RegisterNodeCallback(std::bind(&top::para_proxy::xtop_para_proxy::handle_register_node, this, std::placeholders::_1, std::placeholders::_2))) {
    ...
    for (auto i = 0u; i < m_chain_para_proxys.size(); ++i) {
        auto const & chain_app = m_chain_para_proxys[i];
        chain_app->start();
    }
...
void xtop_para_proxy::stop() {
    for (auto i = m_chain_para_proxys.size(); i > 0; --i) {
        auto const & chain_app = m_chain_para_proxys[i - 1];
        chain_app->stop();
    }
...
common::xnode_id_t const & xtop_para_proxy::node_id() const noexcept {
    return m_node_id;
}

xpublic_key_t const & xtop_para_proxy::public_key() const noexcept {
    return m_public_key;
}

std::string const & xtop_para_proxy::sign_key() const noexcept {
    return m_sign_key;
}

observer_ptr<xbase_timer_driver_t> xtop_para_proxy::timer_driver() const noexcept {
    return make_observer(m_timer_driver.get());
}

observer_ptr<network::xnetwork_driver_face_t> xtop_para_proxy::network_driver(common::xnetwork_id_t const & network_id) const noexcept {
    return make_observer(m_elect_main->GetEcVhost(network_id.value()).get());
}

std::shared_ptr<top::elect::ElectManager> xtop_para_proxy::elect_manager() const noexcept {
    return m_elect_main->GetElectManager();
}

observer_ptr<elect::ElectMain> xtop_para_proxy::elect_main() const noexcept {
    return make_observer(m_elect_main.get());
}

observer_ptr<time::xchain_time_face_t> xtop_para_proxy::logic_timer() const noexcept {
    assert(m_logic_timer != nullptr);
    return make_observer(m_logic_timer.get());
}

observer_ptr<mbus::xmessage_bus_face_t> xtop_para_proxy::message_bus() const noexcept {
    return make_observer(m_bus.get());
}

observer_ptr<store::xstore_face_t> xtop_para_proxy::store() const noexcept {
    return make_observer(m_store.get());
}

observer_ptr<base::xvblockstore_t> xtop_para_proxy::blockstore() const noexcept {
    return make_observer(m_blockstore.get());
}

observer_ptr<router::xrouter_face_t> xtop_para_proxy::router() const noexcept {
    return make_observer(m_router.get());
}

xtop_para_proxy::xthread_pool_t const & xtop_para_proxy::thread_pool(xthread_pool_type_t const thread_pool_type) const noexcept {
...
observer_ptr<xtxpool_v2::xtxpool_face_t> xtop_para_proxy::txpool() const noexcept {
    return make_observer(m_txpool.get());
}

xobject_ptr_t<base::xvnodesrv_t> xtop_para_proxy::node_service() const noexcept {
    return m_nodesvr_ptr;
}

xobject_ptr_t<base::xvcertauth_t> xtop_para_proxy::cert_serivce() const noexcept {
    return m_cert_ptr;
}

xobject_ptr_t<store::xsyncvstore_t> xtop_para_proxy::syncstore() const noexcept {
    return m_syncstore;
}

base::xauto_ptr<top::base::xvblock_t> xtop_para_proxy::last_logic_time() const {
...
bool xtop_para_proxy::check_rootblock() {
    ...
    if (false == blockstore()->store_block(base::xvaccount_t(rootblock->get_account()), rootblock)) {
        xerror("xtop_para_proxy::check_rootblock rootblock set db fail");
        return false;
    }

    xinfo("xtop_para_proxy::check_rootblock success");
    return true;
}

bool xtop_para_proxy::preprocess_accounts_data() {
...
bool xtop_para_proxy::create_genesis_accounts() {
    if (!preprocess_accounts_data()) {
        xwarn("xtop_para_proxy::create_genesis_accounts preprocess_accounts_data failed");
        return false;
    }
    ...
    xinfo("xtop_para_proxy::create_genesis_accounts success");
    return true;
}

bool xtop_para_proxy::create_genesis_account(std::string const & address, uint64_t const init_balance) {
    xdbg("xtop_para_proxy::create_genesis_account address=%s balance=%ld", address.c_str(), init_balance);
    ...
    if (!ret) {
        xerror("xtop_para_proxy::create_genesis_account store genesis block fail");
        return false;
    }
    ret = m_blockstore->execute_block(_vaddr, genesis_block.get());
    if (!ret) {
        xerror("xtop_para_proxy::create_genesis_account execute genesis block fail");
        return false;
    }
    return true;
}

bool xtop_para_proxy::create_genesis_account(std::string const & address, chain_data::data_processor_t const & data) {
    xdbg("xtop_para_proxy::create_genesis_account address=%s balance=%ld burn_balance=%ld tgas_balance=%ld vote_balance=%ld lock_balance=%ld lock_tgas=%ld unvote_num=%ld expire_vote=%ld create_time=%ld lock_token=%ld pledge_vote_str_cnt=%ld",
         address.c_str(),
         data.top_balance,
         data.burn_balance,
         data.tgas_balance,
...
int32_t xtop_para_proxy::handle_register_node(std::string const & node_addr, std::string const & node_sign) {
...
void xtop_para_proxy::create_thread_pools() {}

bool xtop_para_proxy::is_genesis_node() const noexcept {
    ...
    for (auto const & item : seeds) {
        if (user_params.account == common::xnode_id_t{item.m_account}) {
            xinfo("xtop_para_proxy::is_genesis_node is genesis node %s", node_id.c_str());
            return true;
        }
    }

    xwarn("xtop_para_proxy::is_genesis_node not genesis node %s", node_id.c_str());
    return false;
}

bool xtop_para_proxy::is_beacon_account() const noexcept {
    try {
        auto const & user_params = data::xuser_params::get_instance();
        ...
        if (bstate == nullptr) {
            xerror("xtop_para_proxy::is_beacon_account fail-get state.");
            return false;
        }
        ...
        for (auto const & property : property_names) {
            if (false == unitstate.string_get(property, result) || result.empty()) {
                xwarn("xtop_para_proxy::is_beacon_account no property %s", property.c_str());
                continue;
            }
            ...
            if (top::get<bool>(current_group_nodes.find(node_id))) {
                xinfo("xtop_para_proxy::is_beacon_account is genesis node. %s", node_id.c_str());
                return true;
            }
        }

        xwarn("xtop_para_proxy::is_beacon_account not genesis %s", node_id.c_str());
        return false;
    } catch (top::error::xtop_error_t const & eh) {
        std::cerr << "para_proxy start failed. exception: " << eh.what() << "; error: " << eh.code().message() << "; category: " << eh.code().category().name() << std::endl;
        xerror("xpara_proxy_t::is_beacon_account failed with exception: %s; error: %s; category: %s", eh.what(), eh.code().message().c_str(), eh.code().category().name());
        return false;
    } catch (std::exception & eh) {
        std::cerr << "para_proxy start failed. exception: " << eh.what() << std::endl;
        xerror("xpara_proxy_t::is_beacon_account failed with exception: %s", eh.what());
        return false;
    }
}
...
NS_BEG1(std)
std::size_t hash<top::para_proxy::xthread_pool_type_t>::operator()(top::para_proxy::xthread_pool_type_t const type) const noexcept {
    return static_cast<std::size_t>(type);
};

std::size_t hash<top::para_proxy::xio_context_type_t>::operator()(top::para_proxy::xio_context_type_t const type) const noexcept {
    return static_cast<std::size_t>(type);
};
