...
xtop_chain_para_proxy::xtop_chain_para_proxy(observer_ptr<xpara_proxy_t> const &                 para_proxy,
  ...
  : m_para_proxy{para_proxy}
  , m_network_id{network_id}
  , m_elect_client_process{std::make_shared<elect::xelect_client_process>(
        m_network_id,
        m_para_proxy->message_bus(),
        std::bind(&xtop_chain_para_proxy::on_election_data_updated, this, std::placeholders::_1, std::placeholders::_2, std::placeholders::_3),
        m_para_proxy->logic_timer())}
  , m_election_cache_data_accessor{top::make_unique<election::cache::xdata_accessor_t>(network_id, m_para_proxy->logic_timer())}
  , m_vhost{std::make_shared<vnetwork::xvhost_t>(para_proxy->network_driver(m_network_id),
                                                 para_proxy->logic_timer(),
                                                 network_id,
  ...
  , m_sync_obj{top::make_unique<sync::xsync_object_t>(para_proxy->message_bus(), para_proxy->store(), make_observer(m_vhost), blockstore, nodesvr_ptr, cert_ptr,
                                                                                        sync_thread, sync_account_thread_pool, sync_handler_thread_pool)}
  , m_grpc_mgr{top::make_unique<grpcmgr::xgrpc_mgr_t>(m_para_proxy->message_bus(), grpc_thread)}
  , m_cons_mgr{xcons_mgr_builder::build(data::xuser_params::get_instance().account.value(),
                                        m_para_proxy->store(),
                                        m_para_proxy->blockstore(),
                                        m_para_proxy->txpool(),
                                        m_para_proxy->logic_timer(),
                                        m_para_proxy->cert_serivce(),
                                        make_observer(m_election_cache_data_accessor),
                                        m_para_proxy->message_bus(),
                                        m_para_proxy->router())}
  , m_txpool_service_mgr{xtxpool_service_v2::xtxpool_service_mgr_instance::create_xtxpool_service_mgr_inst(m_para_proxy->store(),
                                                                                                        make_observer(m_para_proxy->blockstore().get()),
                                                                                                        m_para_proxy->txpool(),
                                                                                                        m_para_proxy->thread_pool(xthread_pool_type_t::txpool_service),
                                                                                                        m_para_proxy->message_bus(),
                                                                                                        m_para_proxy->logic_timer())}
  , m_vnode_manager{std::make_shared<vnode::xvnode_manager_t>(m_para_proxy->elect_main(),
                                                              m_para_proxy->message_bus(),
                                                              m_para_proxy->store(),
                                                              make_observer(m_para_proxy->blockstore().get()),
                                                              m_para_proxy->logic_timer(),
...
void xtop_chain_para_proxy::start() {
    contract::xcontract_manager_t::instance().install_monitors(
        m_para_proxy->message_bus(), make_observer(m_message_callback_hub.get()), m_para_proxy->store(), m_para_proxy->syncstore());
...
void xtop_chain_para_proxy::stop() {
    m_sync_obj->stop();
...
void xtop_chain_para_proxy::on_election_data_updated(data::election::xelection_result_store_t const & election_result_store,
    ...
    if (!updated_election_data2.empty()) {
        ...
        for (const auto & xip : outdated_xips_pair.first) {
            m_para_proxy->elect_manager()->OnElectQuit(xip);
        }
        ...
        if (zid != common::xfrozen_zone_id) {
            m_para_proxy->elect_manager()->OnElectUpdated(election_result_store, zid);
        }
    } else {
        xinfo("[xchain_para_proxy] network %" PRIu32 " empty election data.  it may be from synchronization", static_cast<std::uint32_t>(m_network_id.value()));
    }
}

common::xnode_id_t const & xtop_chain_para_proxy::node_id() const noexcept {
    return m_para_proxy->node_id();
}

xpublic_key_t const & xtop_chain_para_proxy::public_key() const noexcept {
    return m_para_proxy->public_key();
}

std::string const & xtop_chain_para_proxy::sign_key() const noexcept {
    return m_para_proxy->sign_key();
}

void xtop_chain_para_proxy::top_grpc_init(uint16_t const grpc_port) {
    if (enable_grpc_service()) {
        grpcmgr::grpc_init(m_para_proxy->store().get(), m_para_proxy->blockstore().get(), m_sync_obj.get(), grpc_port);
    }
}

void xtop_chain_para_proxy::top_console_init() {
