...
xtop_beacon_chain_para_proxy::xtop_beacon_chain_para_proxy(observer_ptr<xpara_proxy_t> const & para_proxy,
...
void xtop_beacon_chain_para_proxy::load_last_election_data() {
    ...
    for (const auto & addr : sys_addr) {
        for (auto const & property : data::election::get_property_name_by_addr(common::xaccount_address_t{ addr })) {
            ...
            xwarn("xbeacon_chain_para_proxy::load_last_election_data begin. contract %s; property %s", addr.c_str(), property.c_str());
            xscope_executer_t loading_result_logger{ [&ec, &addr, property] {
                xwarn("xbeacon_chain_para_proxy::load_last_election_data end. contract %s; property %s", addr.c_str(), property.c_str());
            } };

            // only use lightunit
            xobject_ptr_t<base::xvblock_t> latest_vblock = data::xblocktool_t::get_latest_genesis_connectted_lightunit(m_para_proxy->blockstore().get(), addr);
            if (latest_vblock == nullptr) {
                xerror("xtop_beacon_chain_para_proxy::load_last_election_data has no latest lightunit. addr=%s", addr.c_str());
                continue;
            }

            uint64_t block_height = latest_vblock->get_height();
            auto const & last_election_result_store = load_election_data(m_para_proxy->blockstore(),
                                                                         ...
                                                                         ec);
            if (ec) {
                xwarn("xbeacon_chain_para_proxy::load_last_election_data fail to load election data. addr %s; property %s; from height %" PRIu64, addr.c_str(), property.c_str(), block_height);
                continue;
            }
            ...
            if ((addr == sys_contract_rec_elect_rec_addr || addr == sys_contract_rec_elect_zec_addr || addr == sys_contract_zec_elect_consensus_addr) && block_height != 0) {
                ...
                auto const & before_last_election_result_store = load_election_data(m_para_proxy->blockstore(),
                                                                                   common::xaccount_address_t{ addr },
                                                                                   prev_block_height,
                                                                                   property,
                                                                                   prev_block_height,
                ...
                if (!ec) {
                    ...
                    xwarn("xbeacon_chain_para_proxy::load_last_election_data fail to load prev election data. addr %s; property %s; from height %" PRIu64, addr.c_str(), property.c_str(), prev_block_height);
                }
            }
