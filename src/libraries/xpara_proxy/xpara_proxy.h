...
template <>
struct hash<top::para_proxy::xthread_pool_type_t> final {
    std::size_t operator()(top::para_proxy::xthread_pool_type_t const type) const noexcept;
};

template <>
struct hash<top::para_proxy::xio_context_type_t> final {
    std::size_t operator()(top::para_proxy::xio_context_type_t const type) const noexcept;
};
...
class xtop_para_proxy final : public xpara_proxy_face_t<xtop_para_proxy> {
public:
...
private:
    using xbase_t = xpara_proxy_face_t<xtop_para_proxy>;
    ...
    std::vector<std::unique_ptr<xchain_para_proxy_t>> m_chain_para_proxys{};

public:
    xtop_para_proxy(xtop_para_proxy const &) = delete;
    xtop_para_proxy & operator=(xtop_para_proxy const &) = delete;
    xtop_para_proxy(xtop_para_proxy &&) = default;
    xtop_para_proxy & operator=(xtop_para_proxy &&) = default;
    ~xtop_para_proxy() override = default;

    xtop_para_proxy(common::xnode_id_t const & node_id, xpublic_key_t const & public_key, std::string const & sign_key);
...
using xtop_para_proxy_t = xtop_para_proxy;
