...
xtop_top_chain_application::xtop_top_chain_application(observer_ptr<xapplication_t> const & application,
...
void xtop_top_chain_application::load_last_election_data() {
    std::vector<std::string> sys_addr{
                ...
                if ((addr == sys_contract_rec_elect_rec_addr || addr == sys_contract_rec_elect_zec_addr || addr == sys_contract_zec_elect_consensus_addr) && block_height != 0) {
                    ...
                    if (prev_latest_vblock != nullptr) {
                        ...
                        if (!next_to_last_block->get_native_property().native_string_get(property, result_next_to_last) && !result_next_to_last.empty()) {
                            ...
                            xerror("xtop_top_chain_application::load_last_election_data fail-read property.addr=%s height=%ld", addr.c_str(), block_height - 1);
                        }
                    } else {
                        xerror("xtop_top_chain_application::load_last_election_data has no prev lightunit. addr=%s height=%ld", addr.c_str(), block_height);
                    }
                }
                on_election_data_updated(election_result_store, zone_id, block_height);
            } else {
                xerror("xtop_top_chain_application::load_last_election_data fail-read property.addr=%s", addr.c_str());
            }
