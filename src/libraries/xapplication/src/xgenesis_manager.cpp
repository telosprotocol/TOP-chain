...
void xtop_genesis_manager::init_genesis_block() {
    std::error_code ec;
    // step1: root account
    ...
    for (auto const & account : m_contract_accounts) {
        create_genesis_of_contract_account(base::xvaccount_t(account.value()));
        top::error::throw_error(ec);
    }
    ...
    // stepx: release resource used by step2 and step3
...
void xtop_genesis_manager::create_genesis_block(base::xvaccount_t const & account) {
    // if (m_blockstore->exist_genesis_block(account)) {
