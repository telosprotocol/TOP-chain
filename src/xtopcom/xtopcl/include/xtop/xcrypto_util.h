...
public:
    ...
    static std::string make_eth_address_by_assigned_key(std::array<uint8_t, PRI_KEY_LEN> & private_key, uint16_t ledger_id = 0);
    static std::string make_child_eth_address_by_assigned_key(const std::string & parent_addr,
                                                          std::array<uint8_t, PRI_KEY_LEN> & private_key,
                                                          uint16_t ledger_id = 0);
    static std::string make_eth_address_by_random_key(uint16_t ledger_id = 0);
