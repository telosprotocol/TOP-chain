...
base::xauto_ptr<base::xvblock_t> xtop_genesis_manager::create_genesis_of_contract_account(common::xaccount_address_t const & account, xenum_create_src_t src, std::error_code & ec) {
    ...
    xdbg("[xtop_genesis_manager::create_genesis_of_contract_account  1111 ] account %s", account.to_string().c_str());
    if (src == xenum_create_src_t::init && m_blockstore->exist_unit(account.vaccount())) {
        xdbg("[xtop_genesis_manager::create_genesis_of_contract_account  2222 ] account %s", account.to_string().c_str());
        auto const existed_genesis_block = m_blockstore->load_unit(account.vaccount(), (uint64_t)0);
        xdbg("[xtop_genesis_manager::create_genesis_of_contract_account  3333 ] account %s", account.to_string().c_str());
    ...
    xinfo("[xtop_genesis_manager::create_genesis_of_contract_account] account: %s, unit=%s create genesis block success", account.to_string().c_str(),genesis_block->dump().c_str());
