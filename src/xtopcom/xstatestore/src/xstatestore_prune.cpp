...
void xstatestore_prune_t::prune_imp(uint64_t exec_height) {
    ...
    {
        std::lock_guard<std::mutex> l(m_prune_lock);
        // auto pruned_height = prune_exec(from_height, to_height);
...
void xstatestore_prune_t::on_table_block_executed(uint64_t exec_height) {
    ...
    uint64_t prune_table_state_diff = XGET_CONFIG(prune_table_state_diff);
    {
        ...
        xdbg("xstatestore_prune_t::on_table_block_executed table:%s will prune.pruned height:%llu,exec height:%llu isstorage:%d iscons:%d",
             m_table_addr.value().c_str(),
             m_pruned_height,
             exec_height,
             is_storage_node,
...
void xstatestore_prune_t::unitstate_prune_batch(const xaccounts_prune_info_t & accounts_prune_info) {
    for (auto & prune_info : accounts_prune_info.get_prune_info()) {
        ...
        if (upper_height <= account_pruned_height) {
            xwarn("xstatestore_prune_t::unitstate_prune_batch height error.account:%s pruned h:%llu upper h:%llu", account_addr.c_str(), account_pruned_height, upper_height);
        }
        ...
        if (base::xvchain_t::instance().get_xdbstore()->delete_range(begin_delete_key, end_delete_key)) {
            xinfo("xstatestore_prune_t::unitstate_prune_batch unitstate prune succ %s between %s and %s",
                  account_addr.value().c_str(),
                  begin_delete_key.c_str(),
                  end_delete_key.c_str());
            m_statestore_base.set_lowest_executed_block_height(account_addr, upper_height - 1);
        } else {
            xerror("xstatestore_prune_t::unitstate_prune_batch unitstate prune failed %s between %s and %s",
                   account_addr.value().c_str(),
                   begin_delete_key.c_str(),
                   end_delete_key.c_str());
        }
...
xhash256_t xstatestore_prune_t::get_state_root_from_block(base::xvblock_t * block) const {
    return m_statestore_base.get_state_root_from_block(block);
}
...
uint64_t xstatestore_prune_t::prune_exec_storage(uint64_t from_height, uint64_t to_height) {
    ...
    for (; height <= to_height; height++) {
        xobject_ptr_t<base::xvblock_t> commit_block =
            base::xvchain_t::instance().get_xblockstore()->load_block_object(get_account().vaccount(), height, base::enum_xvblock_flag_committed, false);
        if (commit_block == nullptr) {
            xerror("xstatestore_prune_t::prune_exec_storage load block fail.table:%s height:%llu", m_table_addr.value().c_str(), height);
            break;
        }
        prune_info.insert_from_tableblock(commit_block.get());
    }
    ...
    xinfo("xstatestore_prune_t::prune_exec_storage prune tablestate and offdata for table %s from %llu to %llu", m_table_addr.value().c_str(), from_height, height - 1);
...
uint64_t xstatestore_prune_t::prune_exec_storage_and_cons(uint64_t from_height, uint64_t to_height) {
    ...
    for (; height <= to_height; height++) {
        xobject_ptr_t<base::xvblock_t> commit_block =
            base::xvchain_t::instance().get_xblockstore()->load_block_object(get_account().vaccount(), height, base::enum_xvblock_flag_committed, false);
        if (commit_block == nullptr) {
            xerror("xstatestore_prune_t::prune_exec_storage_and_cons load block fail.table:%s height:%llu", m_table_addr.value().c_str(), height);
            break;
        }
        accounts_prune_info.insert_from_tableblock(commit_block.get());
        prune_info.insert_from_tableblock(commit_block.get());
    }
...
uint64_t xstatestore_prune_t::prune_exec_cons(uint64_t from_height, uint64_t to_height) {
    ...
    if (lowest_keep_block == nullptr) {
        xinfo("xstatestore_prune_t::prune_exec_cons table:%s load lowest block fail.height:%llu", get_account().value().c_str(), lowest_keep_height);
        return from_height - 1;
    }

    auto lowest_keep_root = get_state_root_from_block(lowest_keep_block.get());
    ...
    std::error_code ec;
    auto lowest_keep_mpt =
        state_mpt::xtop_state_mpt::create(get_account(), lowest_keep_root, base::xvchain_t::instance().get_xdbstore(), state_mpt::xstate_mpt_cache_t::instance(), ec);
    ...
    for (uint64_t height = from_height; height <= to_height; height++) {
        xobject_ptr_t<base::xvblock_t> commit_block =
            base::xvchain_t::instance().get_xblockstore()->load_block_object(get_account().vaccount(), height, base::enum_xvblock_flag_committed, false);
        if (commit_block == nullptr) {
            xdbg("xstatestore_prune_t::prune_exec_cons load block fail.table:%s height:%llu", m_table_addr.value().c_str(), height);
            continue;
        }
        auto root = get_state_root_from_block(commit_block.get());
        if (root == xhash256_t{}) {
            continue;
        }

        std::error_code ec;
        lowest_keep_mpt->prune(root, ec);
        if (ec) {
            xerror("xstatestore_prune_t::prune_exec_cons prune mpt fail.table:%s,height:%llu,root:%s", m_table_addr.value().c_str(), height, root.as_hex_str().c_str());
        }
        accounts_prune_info.insert_from_tableblock(commit_block.get());
    }
    unitstate_prune_batch(accounts_prune_info);
