...
base::xvblock_ptr_t xstatestore_impl_t::get_unit_block(xblock_number_t number, common::xaccount_address_t const & account_address) const {
    ...
    if (number == LatestConnectBlock || number == LatestBlock || number == PendingBlock) {
        ...
        if (accountindex.get_latest_unit_hash().empty()) {
            unitblock = get_blockstore()->load_unit(account_address.vaccount(), accountindex.get_latest_unit_height());
        } else {
...
base::xauto_ptr<base::xvblock_t> xstatestore_impl_t::get_latest_connectted_state_changed_block(base::xvblockstore_t* blockstore, const base::xvaccount_t & account) const {
    ...
    if (!account_index.get_latest_unit_hash().empty()) {
        ...
        vblock = blockstore->load_unit(account, account_index.get_latest_unit_height());
    }
    if (vblock == nullptr) {
