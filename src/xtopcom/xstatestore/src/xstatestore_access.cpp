...
xtablestate_ext_ptr_t xstatestore_accessor_t::read_table_bstate_from_db(common::xaccount_address_t const& address, base::xvblock_t* block) const {
    ...
    std::error_code ec;
    std::shared_ptr<state_mpt::xstate_mpt_t> mpt = state_mpt::xstate_mpt_t::create(common::xaccount_address_t{block->get_account()}, stateroot, m_store_base.get_dbstore(), ec);
    if (ec) {
