...
void xstatestore_cache_t::set_unitstate(std::string const& block_hash, data::xunitstate_ptr_t const& state) {
    ...
    xinfo("xstatestore_cache_t::set_unitstate hash=%s,state=%s,count=%zu", base::xstring_utl::to_hex(block_hash).c_str(), state->get_bstate()->dump().c_str(), m_unitstates_cache.size());
}

size_t xstatestore_cache_t::get_unitstate_cache_size() const {
    return m_unitstates_cache.size();
}
...
void xstatestore_dbaccess_t::batch_write_unit_bstate(const std::map<std::string, std::string> & batch_kvs, std::error_code & ec) const {    
    if (m_statestore_base.get_dbstore()->set_values(batch_kvs)) {
        xdbg("xstatestore_dbaccess_t::batch_write_unit_bstate succ. size=%zu", batch_kvs.size());
        return;
    }
    ec = error::xerrc_t::statestore_db_write_err;
}
...
xstatestore_accessor_t::xstatestore_accessor_t(common::xtable_address_t const& address) {
    base::xvtable_t * target_table = base::xvchain_t::instance().get_table(address.vaccount().get_xvid()); // TODO(jimmy) the single instance not suitable
...
void xstatestore_accessor_t::write_batch_unitstates_to_db(std::map<std::string, std::string> const& kvs, std::error_code & ec) {
    m_dbaccess.batch_write_unit_bstate(kvs, ec);
}
