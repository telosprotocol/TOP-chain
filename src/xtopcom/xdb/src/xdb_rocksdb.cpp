...
public:
 ...
 public:
    ...
    bool erase(std::vector<xspan_t<char const>> const & keys);
    bool batch_change(const std::map<std::string, std::string>& objs, const std::vector<std::string>& delete_keys);
    bool batch_change(const std::map<std::string, std::string> & objs, std::vector<xspan_t<char const>> const & delete_keys);
 ...
 private:
    rocksdb::ColumnFamilyHandle* get_cf_handle(xspan_t<char const> key) const;
...
rocksdb::ColumnFamilyHandle * xdb::xdb_impl::get_cf_handle(xspan_t<char const> const key) const {
...
bool xdb::xdb_impl::erase(std::vector<xspan_t<char const>> const & keys) {
    rocksdb::WriteBatch batch;
    for (const auto & key : keys) {
        rocksdb::ColumnFamilyHandle * target_cf = get_cf_handle(key);
...
bool xdb::xdb_impl::batch_change(const std::map<std::string, std::string> & objs, std::vector<xspan_t<char const>> const & delete_keys) {
    rocksdb::WriteBatch batch;
    for (const auto & entry : objs) {
        rocksdb::ColumnFamilyHandle * target_cf = get_cf_handle(entry.first);
        XMETRICS_GAUGE(metrics::db_write_size, entry.second.size());
...
bool xdb::batch_change(const std::map<std::string, std::string> & objs, std::vector<xspan_t<char const>> const & delete_keys) {
    return m_db_impl->batch_change(objs, delete_keys);
}
