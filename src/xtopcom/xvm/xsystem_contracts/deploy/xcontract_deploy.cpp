...
void xtop_contract_deploy::deploy_sys_contracts() {
    ...
    if (XGET_CONFIG(enable_reward_contract)) {
    ...
    if (XGET_CONFIG(enable_workload_contract)) {
    ...
    deploy(common::xaccount_address_t{sys_contract_sharding_statistic_info_addr},
           ...
           "",
           enum_broadcast_policy_t::normal,
           std::string(sys_contract_beacon_timer_addr) + ",report_summarized_statistic_info,C," +
               config::xtable_statistic_report_schedule_interval_onchain_goverance_parameter_t::name);
