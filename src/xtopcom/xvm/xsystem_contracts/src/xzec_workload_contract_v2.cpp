...
std::vector<xobject_ptr_t<data::xblock_t>> xzec_workload_contract_v2::get_fullblock(common::xlogic_time_t const timestamp, const uint32_t table_id) {
    ...
    while (true) {
        base::xauto_ptr<data::xblock_t> next_block = get_next_fullblock(table_owner.to_string(), cur_read_height);
        if (nullptr == next_block) {
            xdbg("[xzec_workload_contract_v2::get_fullblock] table %s next full block is empty, last read height: %lu", table_owner.to_string().c_str(), last_read_height);
            break;
        }
        ...
        if (timestamp < next_block->get_clock() || timestamp < (next_block->get_clock() + time_interval)) {
            ...
            xdbg("[xzec_workload_contract_v2::get_fullblock] table: %s, block height: %" PRIu64 " push in", table_owner.to_string().c_str(), block_height);
