...
{
    XMETRICS_TIME_RECORD(XWORKLOAD_CONTRACT "on_receive_reward");
    XMETRICS_COUNTER_INCREMENT(XWORKLOAD_CONTRACT "on_receive_reward", 1);
    ...
    if (sys_contract_sharding_statistic_info_addr != base_addr && sys_contract_eth_table_statistic_info_addr != source_address) {
        xwarn("[xzec_consortium_reward_contract::on_receive_reward] invalid call from %s", source_address.c_str());
        return;
    }
    ...
    xinfo("[xzec_consortium_reward_contract::on_receive_reward] XPORPERTY_CONTRACT_WORKLOAD_KEY workload_str_new size %ld ", workload_str_new.size());
    for (auto it = workload_str_new.cbegin(); it != workload_str_new.end(); it++) {
        xinfo("[xzec_consortium_reward_contract::on_receive_reward] XPORPERTY_CONTRACT_WORKLOAD_KEY add ");
        MAP_SET(XPORPERTY_CONTRACT_WORKLOAD_KEY, it->first, it->second);
    }

    update_table_height(table_id, height_str);
}
...
void xzec_consortium_reward_contract::handle_reward_str(const std::string& table_info_str,
    ...
    std::map<std::string, std::string>& workload_str_new)
{
    XMETRICS_TIME_RECORD(XWORKLOAD_CONTRACT "handle_reward_str");
...
void xzec_consortium_reward_contract::update_reward(const std::map<common::xgroup_address_t, xgroup_cons_reward_t>& group_workload,
    const std::map<std::string, std::string>& workload_str,
    std::map<std::string, std::string>& workload_new)
{
    XMETRICS_TIME_RECORD(XWORKLOAD_CONTRACT "update_reward");
    for (auto const& one_group_workload : group_workload) {
        ...
        // update
        for (auto const& leader_workload : cons_reward.m_leader_count) {
            auto const& leader = leader_workload.first;
            auto const& reward = leader_workload.second;
            total_workload.m_leader_count[leader] += reward;
    ...
    data::system_contract::xissue_detail_v2& issue_detail)
{
    ...
    for (auto it = all_group_rewards.begin(); it != all_group_rewards.end(); it++) {
        ...
        xdbg("[xzec_reward_contract::get_reward_param]  property_param.auditor_workloads_detail workload:");
    }
...
void xzec_reward_contract::calc_nodes_rewards(common::xlogic_time_t const current_time,
...
void xzec_reward_contract::calc_node_rewards(data::system_contract::xreg_node_info const& node,
                                             std::map<common::xgroup_address_t, data::system_contract::xgroup_cons_reward_t> node_reward_detail;
                                             ,
                                             ::uint128_t& reward_to_self)
{
    xdbg("[xzec_reward_contract::calc_auditor_worklaod_rewards] account: %s, node_reward_detail size %d \n",
        node.m_account.c_str(),
        node_reward_detail.size());

    reward_to_self = 0;
    for (auto& workload : node_reward_detail) {
        xdbg("[xzec_reward_contract::calc_auditor_worklaod_rewards] account: %s, group: %s, group size: %d\n",
            node.m_account.c_str(),
            workload.first.to_string().c_str(),
            workload.second.m_leader_reward.size());
        auto it = workload.second.m_leader_reward.find(node.m_account.value());
        if (it != workload.second.m_leader_reward.end()) {
            auto const& reward = it->second;
            reward_to_self += reward;
            xdbg(
                "[xzec_reward_contract::calc_auditor_worklaod_rewards] account: %s, group: %s, reward: %d, new reward: %u \n",
                node.m_account.c_str(),
                workload.first.to_string().c_str(),
                reward,
                reward_to_self);
...
void xzec_reward_contract::update_accumulated_issuance(uint64_t const timer_round)
{
...
bool xzec_consortium_reward_contract::reward_expire_check(const uint64_t onchain_timer_round)
{
    ...
    xinfo("[xzec_consortium_reward_contract::reward_is_expire] will reward, new_time_height %llu, old_time_height %llu, reward_distribute_interval: %u\n",
        new_time_height, old_time_height, reward_distribute_interval);
    return true;
}
