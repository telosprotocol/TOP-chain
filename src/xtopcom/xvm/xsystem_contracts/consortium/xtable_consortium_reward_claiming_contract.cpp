...
void xtop_table_consortium_reward_claiming_contract::recv_node_reward(uint64_t issuance_clock_height, std::map<std::string, ::uint128_t> const & rewards) {
    ...
    XCONTRACT_ENSURE((sys_contract_zec_reward_addr == source_address), "[xtop_table_consortium_reward_claiming_contract::recv_node_reward] working reward is not from zec workload contract but from " + source_address);
