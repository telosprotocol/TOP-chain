    ...
    uint64_t block_height)
{
    ...
    try {
        ...
        xwarn("[xtable_statistic_cons_contract][on_collect_statistic_info_cons] read summarized slash info error:%s", e.what());
        throw;
    }
    ...
    if (block_height <= cur_statistic_height) {
        xwarn("[xtable_statistic_cons_contract][on_collect_statistic_info_cons] duplicated block, block height: %" PRIu64 ", current statistic block %" PRIu64,
            block_height,
            cur_statistic_height);
        return;
    }

    xinfo("[xtable_statistic_cons_contract][on_collect_statistic_info_cons] enter collect statistic data, fullblock height: %" PRIu64 ",contract addr: %s, table_id: %u",
        block_height,
        source_addr.c_str(),
        table_id);
    ...
    try {
        ...
        xwarn("[xtable_statistic_cons_contract][on_collect_statistic_info_cons] read summarized tableblock num error:%s", e.what());
        throw;
    }
        ...
        for (auto const& group_item : elect_statistic.group_statistics_data) {
            ...
            for (size_t slotid = 0; slotid < group_account_data.account_statistics_data.size(); ++slotid) {
                ...
                if (account_reward > 0) {
                    ...
                    it2->second.m_leader_reward[account_str.to_string()] += account_reward;
                }
    ...
    for (auto const& one_group_workload : group_workload) {
        ...
        for (auto const& leader_workload : cons_reward.m_leader_reward) {
            ...
            xdbg("[xtable_statistic_cons_contract::update_reward] group: %u, leader: %s, reward: %d, total_reward: %d",
                ...
                leader.c_str(),
                reward,
                total_reward.m_leader_reward[leader]);
        }
        // set
...
void xtable_statistic_cons_contract::save_reward(common::xgroup_address_t const& group_address, xgroup_cons_reward_t const& group_workload)
{
    ...
    std::string group_address_str = std::string((const char*)key_stream.data(), key_stream.size());
     xdbg("[xtable_statistic_cons_contract][save_reward] group_address_str %s, group_workload size %d ", group_address_str.c_str(), group_workload.m_leader_reward.size());
    xstream_t stream(xcontext_t::instance());
...
void xtable_statistic_cons_contract::report_summarized_statistic_info(common::xlogic_time_t timestamp)
{
...
void xtable_statistic_cons_contract::upload_reward()
{
    ...
    xdbg("[xtable_statistic_cons_contract][upload_reward] group_workload_str size %d \n",group_workload_str.size());
    for (auto it = group_workload_str.begin(); it != group_workload_str.end(); it++) {
        ...
        for (auto const& leader_workload : group_workload.m_leader_reward) {
            ...
            if (it2 == group_workload_upload.end()) {
                xgroup_cons_reward_t empty_workload;
