 ...
 *
 */
class xrec_consortium_proposal_contract final : public xcontract_base {
    using xbase_t = xcontract_base;
public:
    XDECLARE_DELETED_COPY_DEFAULTED_MOVE_SEMANTICS(xrec_consortium_proposal_contract);
    XDECLARE_DEFAULTED_OVERRIDE_DESTRUCTOR(xrec_consortium_proposal_contract);

    explicit
    xrec_consortium_proposal_contract(common::xnetwork_id_t const & network_id);

    xcontract_base*  clone() override {return new xrec_consortium_proposal_contract(network_id());}
    ...
    BEGIN_CONTRACT_WITH_PARAM(xrec_consortium_proposal_contract)
        CONTRACT_FUNCTION_PARAM(xrec_consortium_proposal_contract, submitProposal);
        CONTRACT_FUNCTION_PARAM(xrec_consortium_proposal_contract, withdrawProposal);
        CONTRACT_FUNCTION_PARAM(xrec_consortium_proposal_contract, tccVote);
...
private:
     ...
     * @return false
     */
    bool get_proposal_info(const std::string& proposal_id, top::tcc::proposal_info& proposal);
     ...
     * @return false
     */
    bool is_valid_proposal_type(top::tcc::proposal_type type);
...
// REGISTER_NAME_CONTRACT(sys_contract_rec_tcc_addr, xrec_consortium_proposal_contract);
NS_END3
