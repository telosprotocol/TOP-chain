        ...
        bool     xvtxstore_t::store_txs(xvblock_t * block_ptr,bool store_raw_tx_bin)
        {
            ...
            if(block_ptr->extract_sub_txs(sub_txs))
            {
                ...
                for(auto & v : sub_txs)
                {
                    ...
                    if (txindex_type == base::enum_txindex_type_send)
                    {
                        XMETRICS_COUNTER_INCREMENT("store_send_tx_count", 1);
                    }

                    if (txindex_type == base::enum_txindex_type_confirm)
                    {
                        XMETRICS_COUNTER_INCREMENT("store_confirm_tx_count", 1);
                        ...
                        else
                        {
                            ...
                            static std::atomic<uint64_t> total_count{0};
                            static std::atomic<uint64_t> over_6clock_count{0};
                            static std::atomic<uint64_t> over_9clock_count{0};
                            if (delay_time >= 6)  // 6 clock
                            {
                                XMETRICS_COUNTER_INCREMENT("store_confirmtx_long_time", 1);
