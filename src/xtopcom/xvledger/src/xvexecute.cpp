...
namespace top
{
    namespace base
    {
        ...
        xvalue_t     xvexecute_t::execute_action(xvaction_t & action,xvexecontxt_t & context)
        {
            ...
            if(target_contract)
            {
                ...
                {
                    //xerror("xvexecute_t::execute_entity,get error(%d) to execute contract(%s)->function(%s)",clone_action.get_method_result()->get_error(), action.get_contract_uri().c_str(),action.get_method_name().c_str());
                }
            }
            else
            {
        ...
        xvoutentity_t*  xvexecute_t::execute_entity(const xvinentity_t * input_entity,xvexecontxt_t & context)
        {
            ...
            if(input_entity != NULL)
            {
                ...
                {
                    xvaction_t clone_action(action);
                    xauto_ptr<xvcontract_t> target_contract(load_contract(action));
                    if(target_contract)
                    {
                        clone_action.copy_result(target_contract->execute(clone_action,context));
                        if( (clone_action.get_method_result()->get_type() != xvalue_t::enum_xvalue_type_error) || (clone_action.get_method_result()->get_error() == enum_xcode_successful) )
                        {
                            xinfo("xvexecute_t::execute_entity,successful execute contract(%s)->function(%s)", action.get_contract_uri().c_str(),action.get_method_name().c_str());
                        }
                        else
        ...
        xauto_ptr<xvoutput_t> xvexecute_t::execute_input(const xvinput_t & input,xvexecontxt_t & context) //execute
        {
            ...
            for(auto & ent : input_entities)
            {
                ...
                if(output_ent != NULL)
                    output_entities.emplace_back(output_ent);
