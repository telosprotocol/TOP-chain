...
namespace top
{
    namespace base
    {
        ...
        public:
            virtual xauto_ptr<xvbstate_t> get_block_state(xvblock_t * target_block, const int etag);
            virtual xauto_ptr<xvbstate_t> get_block_state(const xvaccount_t & account,const uint64_t block_height,const std::string& block_hash, const int etag);
            virtual xauto_ptr<xvbstate_t> get_block_state(const xvaccount_t & account,const uint64_t block_height,const uint64_t block_view_id, const int etag);
            virtual xauto_ptr<xvbstate_t> get_latest_connectted_block_state(const xvaccount_t & account, const int etag);
            virtual xauto_ptr<xvbstate_t> get_committed_block_state(const xvaccount_t & account,const uint64_t block_height, const int etag);
            virtual bool                  get_full_block_offsnapshot(xvblock_t * target_block, const int etag);

            virtual xauto_ptr<xvbstate_t> get_block_state_2(xvblock_t * target_block, const int etag);
            virtual bool                  execute_block(xvblock_t * target_block, const int etag);

        private:
            xauto_ptr<xvbstate_t>     load_block_state(const xvbindex_t * target_index, const int etag);
