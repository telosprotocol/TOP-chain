...
NS_BEG2(top, common)
evm_common::xh256_t handle_data(const argument_t & arg);
evm_common::xh256_t handle_indexed(const argument_t & arg);
evm_common::xh256_t to_keccak256_32bytes(const std::string & data);
evm_common::xh256_t to_mapping_top_addr(const xaccount_address_t & top_addr);

xtop_log_t xtop_event_t::pack() {
    common::xtop_log_t log{};
    log.address = address;
    // log topics[0]
    log.topics.emplace_back(evm_common::xh256_t(to_keccak256_32bytes(name)));
...
evm_common::xh256_t handle_indexed(const argument_t & arg) {
    evm_common::xh256_t ret = evm_common::xh256_t{};
    switch (arg.type) {
    case enum_event_data_type::string: {
        ret = evm_common::xh256_t{to_keccak256_32bytes(arg.str_data)};
        break;
    }
    case enum_event_data_type::uint64: {
        ret = evm_common::xh256_t(arg.uint64_t_data);
        break;
    }
    case enum_event_data_type::address: {
        ...
        if (top_addr.type() == base::enum_vaccount_addr_type_secp256k1_eth_user_account || top_addr.type() == base::enum_vaccount_addr_type_secp256k1_evm_user_account) {
            ...
            ret = evm_common::xh256_t(eth_addr.to_h256());
        } else {
            ret = evm_common::xh256_t{to_mapping_top_addr(top_addr)};
        }
        break;
    }
    default:
...
evm_common::xh256_t handle_data(const argument_t & arg) {
    ...
    case enum_event_data_type::string: {
        ...
        xbytes_t head = evm_common::xh256_t{32}.to_bytes();
        xbytes_t len = evm_common::xh256_t{str_data_size}.to_bytes();
...
evm_common::xh256_t to_mapping_top_addr(const xaccount_address_t & top_addr) {
    ...
    return evm_common::xh256_t{ret};
}

evm_common::xh256_t to_keccak256_32bytes(const std::string & data) {
    auto b = top::to_bytes(data);
    auto v = ethash::keccak256(b.data(), b.size());
    return evm_common::xh256_t{xbytes_t{&v.bytes[0], &v.bytes[32]}};
}
