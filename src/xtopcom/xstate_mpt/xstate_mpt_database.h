...
#include "xdbstore/xaccessors_state.h"
...
public:
    xtop_state_mpt_caching_db(db::xkv_db_face_ptr_t db) : m_db(evm_common::trie::xtrie_db_t::NewDatabase(db)), m_code_cache(code_cache_size) {
    }
    ...
    xbytes_t unit(xhash256_t hash, std::error_code & ec) override {
        ...
        v = store::ReadUnit(m_db->DiskDB(), hash);
        if (!v.empty()) {
    ...
    static std::shared_ptr<xtop_state_mpt_caching_db> NewDatabase(db::xkv_db_face_ptr_t diskdb);
    static std::shared_ptr<xtop_state_mpt_caching_db> NewDatabaseWithConfig(db::xkv_db_face_ptr_t diskdb, evm_common::trie::xtrie_db_config_ptr_t config);
