...
private:
    ...
    // std::unordered_map<xhash256_t, uint64_t> trie_root_hash_to_table_height_map_;
    std::map<uint64_t, std::vector<xhash256_t>> table_height_to_root_hashes_map_;
    std::unordered_map<xhash256_t, std::unordered_set<xhash256_t>> trie_node_hashes_;
...
public:
    ...
    void load_state(xhash256_t const & root_hash, base::xvdbstore_t * db, std::error_code & ec);
};
