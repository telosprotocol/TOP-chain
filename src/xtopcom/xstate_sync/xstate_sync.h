...
private:
    ...
    std::set<xh256_t> m_trie_tasks;
    std::map<xh256_t, xbytes_t> m_unit_tasks;
...
private:
    ...
    void fill_tasks(uint32_t total_n, uint32_t unit_n, state_req & req, std::vector<xh256_t> & nodes, std::vector<xbytes_t> & units);
    void process_table(state_req & req, std::error_code & ec);
    void process_trie(state_req & req, std::error_code & ec);
    xh256_t process_node_data(const xbytes_t & blob, std::error_code & ec);
    xh256_t process_unit_data(const xbytes_t & blob, uint8_t version, std::error_code & ec);
