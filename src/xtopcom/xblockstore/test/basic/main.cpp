...
int test_sync_vstore(store::xsyncvstore_t* sync_store)
{
    ...
    base::xauto_ptr<base::xvblock_t> latest_cert_block = sync_store->get_vblockstore()->get_latest_cert_block(test_account_address);
    base::xvblock_t * last_block = latest_cert_block.get();
    generated_blocks[last_block->get_height()] = last_block;
    for(int i = 1; i < total_test_blocks; ++i)
    {
        xunitblock_t * new_block = NULL;
        const uint32_t random = top::base::xtime_utl::get_fast_randomu();
        if( (random % 2) == 0)
            new_block = xunitblock_t::create_unitblock(test_account_address, last_block->get_height()+1, last_block->get_clock()+1, last_block->get_viewid()+1, last_block->get_block_hash(), generated_blocks[0]->get_block_hash(), 0, empty_input_output, empty_input_output);
        else
            new_block = xunitblock_t::create_unitblock(test_account_address, last_block->get_height()+1, last_block->get_clock()+1, last_block->get_viewid()+1, last_block->get_block_hash(), generated_blocks[0]->get_block_hash(), 0, test_input_output, test_input_output);
        ...
        generated_blocks[new_block->get_height()] = new_block;
