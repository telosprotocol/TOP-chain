...
namespace top
{
    namespace store
    {
        ...
        #define LOAD_BLOCKACCOUNT_PLUGIN(account_obj,account_vid,default_val) \
            if(is_close())\
            {\
    ...
    //                xassert(target_index->get_block_flags() == target_index->get_this_block()->get_block_flags());  // TODO(jimmy)
        ...
        {
            LOAD_BLOCKACCOUNT_PLUGIN(account_obj,account,nullptr);
        ...
        bool                xvblockstore_impl::load_block_input(const base::xvaccount_t & account,base::xvblock_t* block,const int atag)
        {
            ...
            LOAD_BLOCKACCOUNT_PLUGIN(account_obj,account,false);
            METRICS_TAG(atag, 1);
