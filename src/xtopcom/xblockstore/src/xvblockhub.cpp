...
namespace top
{
    namespace store
    {
        ...
        {
            return clean_blocks(0);//try all possible clean_caches
        }

        bool xblockacct_t::clean_blocks(size_t keep_blocks_count)
        {
            ...
            else if(0)
            {
                for(auto height_it = m_all_blocks.begin(); height_it != m_all_blocks.end();)//search from lowest hight to higher
                {
                    ...
                    if(old_height_it->first != 0) //clean non genesis here
                    {
                        auto & view_map = old_height_it->second;
                        for(auto it = view_map.begin(); it != view_map.end(); ++it)
                        {
            ...
            if(false == m_all_blocks.empty())
            {
                for(auto height_it = m_all_blocks.begin(); height_it != m_all_blocks.end(); ++height_it)//search from lower height
                {
                    ...
                    for(auto view_it = view_map.begin(); view_it != view_map.end(); ++view_it) //search from lower view#
                    {
                        ...
                        mark_connected_flag(view_it->second);
        ...
        base::xvbindex_t*  xblockacct_t::load_latest_connected_index() //block has connected to genesis or latest full-block
        {
            ...
            {
                xwarn("xblockacct_t::load_latest_connected_index,fail load block at height(%llu) of account(%s)",m_meta->_highest_connect_block_height,get_address().c_str());
        ...
        base::xvbindex_t*  xblockacct_t::load_latest_genesis_connected_index() //block has connected to genesis
        {
            ...
            {
                xwarn("xblockacct_t::load_latest_genesis_connected_index,fail load block at height(%llu) of account(%s)",m_meta->_highest_genesis_connect_height,get_address().c_str());
        ...
        int   xblockacct_t::load_index(const uint64_t target_height)
        {
            ...
            {
                std::map<uint64_t,base::xvbindex_t*> _indexes(read_index_from_db(target_height));
                if(_indexes.empty() == false) //found index at db
                {
                    for(auto it = _indexes.begin(); it != _indexes.end(); ++it)
                    {
                        ...
                        process_index(it->second);    //process it then
                ...
                xwarn("xblockacct_t::load_index(),fail found index for addr=%s at height=%llu", get_account().c_str(), target_height);
                return 0;
            }
        ...
        base::xvbindex_t*     xblockacct_t::load_index(const uint64_t target_height,const uint64_t view_id)
        {
            ...
            if(NULL == target_block)
                ...
                clean_blocks(enum_max_cached_blocks);
        ...
        bool    xblockacct_t::store_block(base::xvblock_t* new_raw_block)
        {
            ...
            {
                new_index_ptr->reset_this_block(new_raw_block); //paired now
                ...
                write_block_to_db(new_index_ptr); //save raw block then

                if(0 != new_raw_block->get_height()) //keep genesis block
                    new_index_ptr->reset_this_block(NULL); //reset null to release memory of raw block
        ...
        bool    xblockacct_t::cache_index(base::xvbindex_t* this_block)
        {
            ...
            if(this_block->check_block_flag(base::enum_xvblock_flag_committed))   //commit block
            {
                ...
                for(auto it = view_map.begin(); it != view_map.end();)
                {
                    ...
                    {
                        xdbg("xblockacct_t::cache_index,new-commit one clean existing block=%s",old_it->second->dump().c_str());
            ...
            else if(this_block->check_block_flag(base::enum_xvblock_flag_locked)) //lock-only block
            {
                for(auto it = view_map.begin(); it != view_map.end();)
                {
                    ...
                    if(old_it->second->check_block_flag(base::enum_xvblock_flag_committed))
                    {
                        xwarn("xblockacct_t::cache_index,lock-only block conflict with existing committed block,block=%s",this_block->dump().c_str());
                        return false; //apply rule#2,not allow put lower stage one
                    }
                    else if(old_it->second->check_block_flag(base::enum_xvblock_flag_locked))
                    {
                        if(old_it->second->get_viewid() == this_block->get_viewid()) //exactly same viewid
                            existing_view_iterator = old_it; //found existing lock block,then reuse this slot
                    }
                    else //clean any cert-only block
                    {
                        xdbg("xblockacct_t::cache_index,new-lock one clean existing block=%s",old_it->second->dump().c_str());
            ...
            else if(this_block->check_block_flag(base::enum_xvblock_flag_authenticated))//cert-only
            {
                for(auto it = view_map.begin(); it != view_map.end();)
                {
                    ...
                    {
                        xwarn("xblockacct_t::cache_index,cert-only block conflict with existing locked/committed block,block=%s",this_block->dump().c_str());
                        return false; //apply rule#2,not allow put lower stage one
                    }
            ...
            if(existing_view_iterator != view_map.end())//apple rule#2 by reuse existing iterator and replace by new value
            {
                ...
                if(this_block != existing_block) //found same one but modified
                {
                    ...
                    existing_block->reset_block_flags(existing_block_flags | new_block_flags);//merge flags
            ...
            else //insert a brand new entry
            {
                ...
                mark_connected_flag(this_block); //update connected flag if have
                update_meta_metric(this_block); //update metric then
        ...
        bool  xblockacct_t::mark_connected_flag(base::xvbindex_t* this_block)
        {
            ...
            if( (this_block->check_block_flag(base::enum_xvblock_flag_connected)) && (this_block_height >= m_meta->_highest_connect_block_height) )
            {
                ...
                for(;heigh_it != m_all_blocks.end();++heigh_it)
                {
                    ...
                    {
                        for(auto view_it = heigh_it->second.rbegin(); view_it != heigh_it->second.rend();++view_it)
                        {
        ...
        bool xblockacct_t::update_meta_metric(base::xvbindex_t* new_block_ptr )
        {
            ...
            if(new_block_ptr->check_block_flag(base::enum_xvblock_flag_committed))
            {
                ...
                {
                    if (   (0 == new_block_height)
                        || (1 == new_block_height && !m_meta->_highest_genesis_connect_hash.empty()))//XTODO,confirm
                    {
                        // after update by genesis block
                        m_meta->_highest_genesis_connect_height = new_block_height;
                        m_meta->_highest_genesis_connect_hash   = new_block_ptr->get_block_hash();
                    }
                }
                else if (new_block_height == m_meta->_highest_genesis_connect_height + 1)
                {
                    if (m_meta->_highest_genesis_connect_hash == new_block_ptr->get_last_block_hash())
                    {
                        m_meta->_highest_genesis_connect_height = new_block_height;
                        m_meta->_highest_genesis_connect_hash   = new_block_ptr->get_block_hash();
                    }
                    ...
                    {
                        xerror("xblockacct_t::update_meta_metric, commit block hash not match. genesis_connect_block_hash=%s,block=%s", base::xstring_utl::to_hex(m_meta->_highest_genesis_connect_hash).c_str(), new_block_ptr->dump().c_str());
                    }
                }
            }
        ...
        bool    xblockacct_t::write_block_to_db(base::xvbindex_t* index_ptr)
        {
            ...
            if(block_ptr->get_block_version_major() < 1) //lower version has stored everthing inside of block object
            {
            ...
            else //new version of block may serialize seperately
            {
                //XTODO, force store everything temporary
                bool ask_store_input  = true;
                bool ask_store_output = true;
                if(  (block_ptr->get_header()->get_block_class() == base::enum_xvblock_class_full) //store everything for full block
                   ||(block_ptr->get_header()->get_block_level() > base::enum_xvblock_level_unit) //for any container block
                   )
                {
                    if(block_ptr->get_block_class() != base::enum_xvblock_class_nil) //not nil block
                    {
                        ask_store_input  = true;
                        ask_store_output = true;
                    }
                }
                if(ask_store_input && (block_ptr->get_input() != NULL) )
                {
                    ...
                    {
                        //XTODO for case of seperate input
                        index_ptr->set_store_flag(base::enum_index_store_flag_input_entity);
                        index_ptr->set_store_flag(base::enum_index_store_flag_input_resource);
                        xdbg("xblockacct_t::write_block_to_db,store input to DB for block(%s)",index_ptr->dump().c_str());
                    }
                    else
                    {
                        xerror("xblockacct_t::store_block_to_db,fail to store input for block(%s)",index_ptr->dump().c_str());
                        return false;
                    }
                }
                if(ask_store_output && (block_ptr->get_output() != NULL) )
                {
                    ...
                    if(base::xvchain_t::instance().get_xdbstore()->set_value(output_key, output_bin))
                    {
                        ...
                        xdbg("xblockacct_t::write_block_to_db,store output to DB for block(%s)",index_ptr->dump().c_str());
                    }
                    else
                    {
                        xerror("xblockacct_t::store_block_to_db,fail to store output for block(%s)",index_ptr->dump().c_str());
                        return false;
                    }
                }
            }
        ...
        bool    xblockacct_t::read_block_input_from_db(base::xvblock_t * block_ptr,const int stored_flags)
        {
            ...
            if(block_ptr->get_input() == NULL)
            {
                ...
                {
                    xinfo("xblockacct_t::read_block_object_from_db,NOT writed block-input yet,index(%s) ",block_ptr->dump().c_str());
                }
            }
            if(block_ptr->get_input() != NULL) //now has valid input
            {
                /*no-need for resource that has been read at above
                   ...
                   &&(block_ptr->get_input()->has_resource_data() == false) ) //but dont have resource avaiable now
                {
                    if(stored_flags & base::enum_index_store_flag_input_resource) //has input resource stored
                    {
                        ...
                        if(block_ptr->get_input()->set_resources_data(input_resource_bin) == false)
                        {
                            xerror("xblockacct_t::load_block_input_from_db,load bad input-resource for key(%s)",input_resource_key.c_str());
                            return false;
                        }
                    }
                    else
                    {
                        xinfo("xblockacct_t::read_block_object_from_db,NOT writed input-resource yet,index(%s) ",index_ptr->dump().c_str());
                    }
                }
                 */
            }
        ...
        bool    xblockacct_t::read_block_output_from_db(base::xvblock_t * block_ptr,const int stored_flags)
        {
            ...
            {
                if(stored_flags & base::enum_index_store_flag_output_entity) //has input stored
                {
                    const std::string output_key = create_block_db_key(block_ptr->get_height(),block_ptr->get_output_hash());
                ...
                {
                    xinfo("xblockacct_t::read_block_output_from_db,NOT writed block-output yet,index(%s) ",block_ptr->dump().c_str());
                }
            }
            if(block_ptr->get_output() != NULL) //now has valid input
            {
                   ...
                   &&(block_ptr->get_output()->has_resource_data() == false) ) //but dont have resource avaiable now
                {
                    if(stored_flags & base::enum_index_store_flag_output_resource) //has output resource stored
                    {
                        ...
                        if(block_ptr->get_output()->set_resources_data(output_resource_bin) == false)
                        {
                    ...
                    {
                        xinfo("xblockacct_t::read_block_output_from_db,NOT stored output-resource yet,index(%s) ",index_ptr->dump().c_str());
                    }
                }
                */
            }
        ...
        bool   xblockacct_t::write_index_to_db(base::xvbindex_t* index_obj)
        {
            ...
            {
                xerror("xblockacct_t::write_index_to_db,passin wrong index(%llu) that not belong to this account(%s)",index_obj->get_account_id(),get_account().c_str());
                return false;
            }
        ...
        std::map<uint64_t,base::xvbindex_t*>   xblockacct_t::read_index_from_db(const uint64_t target_height)
        {
            std::map<uint64_t,base::xvbindex_t*> all_blocks_at_height;
            ...
            {
                xwarn("xblockacct_t::read_index_from_db,dont find main entry for height(%llu)",target_height);
                return all_blocks_at_height;
            }
            ...
            all_blocks_at_height[index_entry->get_viewid()] = index_entry;//transfer owner to map
