...
namespace top
{
    namespace store
    {
        ...
        public: //return raw ptr with added reference,caller respond to release it after that.
            ...
            virtual base::xauto_ptr<base::xvblock_t>  get_genesis_current_block(const std::string & account) override;//block connected to genesis
            virtual base::xauto_ptr<base::xvblock_t>  get_highest_sync_block(const std::string & account) override;
            virtual int                               get_cache_size(const std::string & account) override;
            ...
            virtual base::xauto_ptr<base::xvblock_t>  load_block_object_without_cache(const std::string & account,const uint64_t height,bool ask_full_load = true) override;
