...
namespace top
{
    namespace store
    {
        ...
        public:
            ...
            {
                block_height = m_meta->_highest_execute_block_height;
                block_hash   = m_meta->_highest_execute_block_hash;
                return true;
            }
            
            bool   set_unit_latest_executed_block(const uint64_t new_height,const std::string & new_block_hash)
            {
                ...
                if(  (new_height > m_meta->_highest_execute_block_height)
                   ||(0 == m_meta->_highest_execute_block_height) )
                {
                    m_meta->_highest_execute_block_height = new_height;
                    m_meta->_highest_execute_block_hash   = new_block_hash;
                    return true;
                }
                return false;
            }
