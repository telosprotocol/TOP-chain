...
public:
    ...
    static bool is_selected_receipt_pull_msg_receiver(uint64_t hash, uint64_t now, uint16_t node_id, uint16_t shard_size);
    static bool is_selected_pos(uint32_t pos, uint32_t rand_pos, uint32_t select_num, uint32_t size);
    static bool is_time_for_node_pull_lacking_receipts(uint64_t now, uint32_t table_id, uint16_t shard_size, uint16_t self_node_id);
};
