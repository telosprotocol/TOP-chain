...
template <typename ActionT>
std::unique_ptr<xaction_session_t<ActionT>> xtop_action_runtime<ActionT>::new_session(observer_ptr<contract_common::xcontract_state_t> contract_state) {
    return top::make_unique<xaction_session_t<ActionT>>(top::make_observer(this), contract_state);
}

//template <data::xtop_action_type_t ActionT>
