...
bool SetLocalDefaultKadKey(base::KadmliaKeyPtr kad_key) {
    kad_key->set_xinterface_id(top::RandomUint32() % std::numeric_limits<uint32_t>::max());
...
std::shared_ptr<top::wrouter::RootRoutingManager> CreateRootManager(
        ...
        const std::shared_ptr<top::db::xdb>& net_db) {
    ...
    base::KadmliaKeyPtr kad_key_ptr = nullptr;
    if (KadKey_GetFromDb(
            kad_key_ptr,
            net_db,
    ...
    auto get_cache_callback = [](const uint64_t& service_type, VecBootstrapEndpoint& vec_bootstrap_endpoint) -> bool {
        return true;
    };
    auto set_cache_callback = [](const uint64_t& service_type, const VecBootstrapEndpoint& vec_bootstrap_endpoint) -> bool {
        return true;
    };
...
bool RoutingTableJoin(
    ...
    if (join_endpoints.empty()) {
        TOP_WARN("no bootstrap endpoint, join failed");
        return false;
    }
...
bool CreateRoutingTable(
    ...
    routing_table_ptr.reset(new top::DemoRouting(udp_transport, local_node_ptr, 256));
...
bool InitRoutingTable(const top::base::Config& config, const std::shared_ptr<top::db::xdb>& net_db) {
    ...
    InitWrouter(udp_transport, support_random_send);
    ...
    auto root_manager_ptr = CreateRootManager(udp_transport, config, public_endpoints_config, net_db);
    if (!root_manager_ptr) {
...
bool ResetEdgeConfig(top::ArgsParser& args_parser, top::base::Config& edge_config) {
    ...
    if (args_parser.GetParam("x", local_xid) == top::kadmlia::kKadSuccess) {
        ...
        if (!local_xid.empty()) {
            if (!edge_config.Set("node", "local_xid", local_xid)) {
                TOP_WARN("set config failed [node][local_xid][%s]", local_xid.c_str());
                return false;
            }
        }
    }
    ...
    if (!edge_config.Set("node", "business", business)) {
        TOP_WARN("set config failed [node][business][%s]", business.c_str());
        return false;
    }
    ...
    if (args_parser.GetParam("s", service_list) == top::kadmlia::kKadSuccess) {
        if (!edge_config.Set("role", "service_list", service_list)) {
            TOP_WARN("set config failed [edge][service_list][%s]", service_list.c_str());
            return false;
        }
...
std::shared_ptr<top::db::xdb> InitDb(const base::Config& config) {
    std::string db_path;
    if (!config.Get("db", "path", db_path)) {
        TOP_FATAL("get db path from conf failed[%s]", db_path.c_str());
        return nullptr;
    }
    auto net_db = std::make_shared<db::xdb>(db_path);
    if (!net_db) {
...
int MainEdge(const top::base::Config& edge_config) {
    ...
    std::shared_ptr<top::db::xdb> net_db = top::InitDb(edge_config);
    if (!net_db) {
    ...
    net_db->write(top::kadmlia::LOCAL_EDGE_DB_KEY+"ip", local_ip);
    ...
    net_db->write(top::kadmlia::LOCAL_COUNTRY_DB_KEY + "code", country);
    ...
    if (!InitRoutingTable(edge_config, net_db)) {
...
int KadKey_GetFromDb(
        ...
        std::shared_ptr<top::db::xdb> net_db,
    ...
    if (!net_db->read(db_key, kad_key_str_hex)) {
...
int KadKey_StoreInDb(
    ...
    net_db->write(db_key, kad_key_str_hex);
    return 0;
}
...
int main(int argc, char** argv) {
    ...
    global_node_id_hash = GetStringSha128(global_node_id);
