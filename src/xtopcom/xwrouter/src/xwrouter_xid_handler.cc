...
int32_t WrouterXidHandler::SendGeneral(transport::protobuf::RoutingMessage& message) {
    ...
    if (message.has_is_root() && message.is_root()) {
        ...
        if (!routing_table) {
            ...
            TOP_DEBUG("crossing network from local_xid: %s to des: %llu",
                    HexEncode(global_xid->Get()).c_str(),
                    service_type);

            std::vector<kadmlia::NodeInfoPtr> des_nodes;
            kadmlia::NodeInfoPtr des_node_ptr;
            if (!wrouter::ServiceNodes::Instance()->GetRootNodes(
                        service_type, message.des_node_id(), des_node_ptr) || !des_node_ptr) {
...
int32_t WrouterXidHandler::SendMulticast(transport::protobuf::RoutingMessage& message) {
    ...
    if (!routing_table || routing_table->nodes_size() == 0) {
        TOP_DEBUG("crossing network from local_xid: %s to des: %llu %s",
                HexEncode(global_xid->Get()).c_str(),
                des_service_type,
        ...
        if (!wrouter::ServiceNodes::Instance()->GetRootNodes(des_service_type, des_nodes) || des_nodes.empty()) {
            TOP_WARN("crossing network failed, can't find des nodes of service_type: %llu %s",
                    des_service_type,
                    HexEncode(message.des_node_id()).c_str());
            return enum_xerror_code_fail;
        }
        ...
        TOP_DEBUG("msg.des: %s crossing network using routing: %s",
                HexEncode(message.des_node_id()).c_str(),
...
int32_t WrouterXidHandler::SendGossip(transport::protobuf::RoutingMessage& message) {
    ...
    // for kRoot, root_service_type maybe invalid
    uint64_t root_service_type = ParserServiceType(message.des_node_id());
    RoutingTablePtr routing_table = FindRoutingTable(true, root_service_type, true, message.des_node_id());
    if (!routing_table) {
...
int32_t WrouterXidHandler::GossipBroadcast(
    ...
    case kGossipSetFilterAndLayered:
        set_layer_gossip_ptr_->Broadcast(
                ...
                neighbors);
        break;
    case kGossipBloomfilterMerge:
        bloom_gossip_merge_ptr_->Broadcast(
                routing_table->get_local_node_info()->hash64(),
                message,
                neighbors);
        break;
    case kGossipBloomfilterZone:
        bloom_gossip_zone_ptr_->Broadcast(
                routing_table->get_local_node_info()->hash64(),
                message,
                neighbors);
        break;
    case kGossipBloomfilterSuperNode:
        bloom_gossip_super_ptr_->Broadcast(message,routing_table);
        break;
    default:
