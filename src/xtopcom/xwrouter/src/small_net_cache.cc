...
bool SmallNetNodes::Init() {
    clear_timer_ = std::make_shared<base::TimerRepeated>(base::TimerManager::Instance(),
            "SmallNetNodes::Clear");
    clear_timer_->Start(
            500ll * 1000ll,
            kClearPeriod,
    ...
    clear_markexpired_timer_ = std::make_shared<base::TimerRepeated>(base::TimerManager::Instance(),
            "SmallNetNodes::ClearMarkExpired");
    clear_markexpired_timer_->Start(
            500ll * 1000ll,
            kClearMarkExpiredPeriod,
            std::bind(&SmallNetNodes::do_clear_for_mark_expired, this));
...
bool SmallNetNodes::GetAdvanceOfConsensus(
        base::KadmliaKeyPtr con_kad,
        uint8_t associated_gid,
    ...
    TOP_DEBUG("GetAdvance %llu of con: %s %llu",
            adv_service_type,
            HexEncode(con_kad->Get()).c_str(),
            con_kad->GetServiceType());
    return true;
}
    ...
    for (auto& rt : vec_rt) {
        ...
        switch (xnetwork_id) {
            case kChainConsensusGroup:
                ...
                uint64_t adv_service_type;
                if (!GetAdvanceOfConsensus(rt->get_local_node_info()->kadmlia_key(),
                            local_node.m_associated_gid, adv_service_type)) {
                    break;
                }
                ...
                TOP_DEBUG("insert relationship service type, local: %llu des: %llu",
                        local_service_type,
                        adv_service_type);
                break;
...
bool SmallNetNodes::FindSuperNode(
    ...
    auto& nodes = ifind->second->nodes;
    if (nodes.empty()) {
        return false;
    }

    // always using the smallest 3 nodes as supernode in each one of p2p-networks
    std::sort(nodes.begin(), nodes.end(), NetNodeCmp());
    auto min_num = std::min(SingleNetSuperNodeSize, static_cast<uint32_t>(nodes.size()));
    for (uint32_t i = 0; i < min_num; ++i) {
        node_ary[i] = nodes[i];
        TOP_DEBUG("smallnet vrf supernode:%s account:%s of service_type:%llu",
                HexEncode(nodes[i].m_node_id).c_str(),
                nodes[i].m_account.c_str(),
                service_type);
    }
...
void SmallNetNodes::AddNodeLimit(
        uint64_t service_type,
...
void SmallNetNodes::AddNodeMarkEnd() {
    // trigger super node info search
    service_nodes_->GetAllSuperRootNodes();
    TOP_DEBUG("smallnet trigger supernode building");
}
...
uint32_t SmallNetNodes::AddNode(NetNode node) {
    ...
    if (node.m_account == global_node_id) {
        ...
        TOP_DEBUG("self addnode account: %s service_type: %llu",
    ...
    if (ifind == net_nodes_cache_map_.end()) {
        ...
        TOP_DEBUG("bluever %ld add node(%s) version(%llu)",
                (long)service_type,
                node.m_account.c_str(),
                node.m_version);
    } else {
        ...
        // filter old version
        if (node.m_version < latest_version) {
            TOP_DEBUG("bluever %ld old version(%llu) node(%s) is ignore, latest version is %llu",
                    (long)service_type, node.m_version, node.m_account.c_str(), latest_version);
            return 0;
        }
    ...
    TOP_DEBUG("addnode account:%s public_key:%s xip:%s service_type:%llu xnetwork_id:%u,"
            "now size:%u",
            node.m_account.c_str(),
...
void SmallNetNodes::HandleExpired(
        std::unordered_map<uint64_t, std::vector<std::string>>& expired_vec,
    ...
    {
        std::unique_lock<std::mutex> mlock(mark_expired_netnode_vec_mutex_);
        mark_expired_netnode_vec_.insert(
                mark_expired_netnode_vec_.end(),
                mark_expired_netnode_vec.begin(),
                mark_expired_netnode_vec.end());
    ...
    for (auto& service_type : unreg_service_type_vec) {
        // TODO(smaug) 
        UnregisterRoutingTable(service_type);
    ...
    for (auto& item :expired_vec) {
        ...
        TOP_INFO("service_node_cache bulkdropnode %u nodes of service_type:%llu",
                (item.second).size(),
                item.first);
    }
        ...
        for (auto& mitem: net_nodes_cache_map_) {
            ...
            for (auto iter = (mitem.second)->nodes.begin(); iter != (mitem.second)->nodes.end(); ) {
                if (iter->m_version < latest_version - 1) {  // keep the latest 2 versions
                    // elect nodes expired
                    TOP_DEBUG("bluever %ld remove expired node(%s) version(%llu)", (long)mitem.first
                            , iter->m_account.c_str(), iter->m_version);
