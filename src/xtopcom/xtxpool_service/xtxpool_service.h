...
using xtxpool_service::xcons_utl;
...
class xtxpool_confirm_receipt_msg_t : public top::basic::xserialize_face_t {
 public:
    ...
    const std::string get_source_addr() const {return m_source_addr;}
...
public:
    explicit xtxpool_service(const observer_ptr<router::xrouter_face_t> &               router,
...
public:
    ...
    void    send_receipt(const xcons_transaction_ptr_t & cons_tx, uint32_t resend_time) override;
