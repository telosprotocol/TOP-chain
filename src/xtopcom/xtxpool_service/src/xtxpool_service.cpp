...
public:
    txpool_receipt_message_para_t(vnetwork::xvnode_address_t const & sender, vnetwork::xmessage_t const & message):
    m_sender(sender),
    m_message(message) {
    }
...
xtxpool_service::xtxpool_service(const observer_ptr<router::xrouter_face_t> & router,
                                 const observer_ptr<store::xstore_face_t> & store,
...
void xtxpool_service::set_params(const xvip2_t & xip,
                                 const std::shared_ptr<vnetwork::xvnetwork_driver_face_t> & vnet_driver) {
...
void xtxpool_service::on_message_receipt(vnetwork::xvnode_address_t const & sender, vnetwork::xmessage_t const & message) {
    ...
    if (message.id() == xtxpool_msg_send_receipt || message.id() == xtxpool_msg_recv_receipt) {
        ...
        if (m_txpool->is_mailbox_over_limit()) {
...
void xtxpool_service::on_message_unit_receipt(vnetwork::xvnode_address_t const & sender, vnetwork::xmessage_t const & message) {
    ...
    if (message.id() == xtxpool_msg_recv_receipt) {
        ...
        auto tx = m_txpool->get_unconfirm_tx(confirm_receipt_msg->get_source_addr(), confirm_receipt_msg->get_receipt()->get_tx_info()->get_tx_hash_256());
        if (tx == nullptr) {
            xwarn("xtxpool_service::on_message_unit_receipt unconfirm tx not found:source addr:%s, hash:%s", confirm_receipt_msg->get_source_addr().c_str(),
...
void xtxpool_service::check_and_response_recv_receipt(const xcons_transaction_ptr_t & cons_tx) {
    ...
    xtransaction_store_ptr_t tx_store = m_store->query_transaction_store(tx->digest());
    ...
    if (tx_store != nullptr && tx_store->m_recv_unit_height != 0) {
        ...
        base::xauto_ptr<base::xvblock_t> blockobj = m_blockstore->load_block_object(tx->get_target_addr(), tx_store->m_recv_unit_height);
        if (blockobj != nullptr) {
            ...
            base::xauto_ptr<base::xvblock_t> justify_tableblock = xblocktool_t::load_justify_block(m_blockstore.get(), table_account, justify_table_height);
            if (justify_tableblock == nullptr) {
                xwarn("xtxpool_service::check_and_response_recv_receipt can not load justify tableblock. tx=%s,account=%s,height=%ld",
                    tx->dump().c_str(), table_account.c_str(), justify_table_height);
                return;
            }
...
// TODO(Nathan):batch process receipt txs
#if 0
    for (auto & v : receipt->m_receipt_txs) {
        for (auto & one_tx : v.m_txs_info) {
            if (!one_tx.m_tx_info.is_send_tx()) {
                xassert(0);
                continue;
            }

            const xtransaction_ptr_t & receipt_tx = one_tx.m_tx;
            xtransaction_store_ptr_t tx_store = m_store->query_transaction_store(receipt_tx->digest());
            // first time consensus transaction has been stored, so it can be found
            // in the second consensus, need check the m_recv_unit_height
            if (tx_store != nullptr && tx_store->m_recv_unit_height != 0) {
                xdbg("send tx receipt has been consensused, txhash:%s", receipt_tx->get_digest_hex_str().c_str());
                xblock_ptr_t unit = m_store->get_block_by_height(data::enum_xblock_type::xblock_type_unit, receipt_tx->get_target_addr(), tx_store->m_recv_unit_height);
                if (unit != nullptr) {
                    xreceipt_unit_ptr_t recv_tx_receipt = xreceipt_maker_t::make_recv_receipt(receipt_tx, unit);
                    send_unit_receipt(recv_tx_receipt);
                } else {
                    xerror("recv tx unit not exist txhash:%s block_height:%d",
                        receipt_tx->get_digest_hex_str().c_str(), tx_store->m_recv_unit_height);
                }
            }
        }
    }
...
void xtxpool_service::send_receipt(const data::xcons_transaction_ptr_t & cons_tx, uint32_t resend_time) {
    if (!m_running) {
        return;
    }
...
void xtxpool_service::auditor_forward_receipt_to_shard(const xcons_transaction_ptr_t & cons_tx, vnetwork::xmessage_t const & message) {
    ...
    bool has_right = has_receipt_right(cons_tx->get_transaction()->digest(), xtxpool::get_receipt_resend_time(cons_tx->get_unit_cert()->get_gmtime(), now));
    if (has_right) {
...
int32_t xtxpool_service::request_transaction_consensus(const data::xtransaction_ptr_t & tx, bool local) {
    xdbg("xtxpool_service::request_transaction_consensus in, tx:source:%s target:%s hash:%s",
         ...
         tx->get_digest_hex_str().c_str());
    if (!m_running) {
        xwarn("[xtxpool_service]not running, tx dropped:source:%s target:%s hash:%s",
              tx->get_source_addr().c_str(),
              tx->get_target_addr().c_str(),
              tx->get_digest_hex_str().c_str());
        return false;
    ...
    if (ret) {
        xwarn("[global_trace][xtxpool_service]tx=%s,account=%s verify send tx source fail",
               tx->get_digest_hex_str().c_str(),
               tx->get_source_addr().c_str());
        return ret;
    }
