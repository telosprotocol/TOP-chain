...
xaccount_ptr_t xstore::query_account(const std::string &address) const {
    ...
    uint64_t  latest_connected_block_height = 0;
    std::string latest_connected_block_hash;
    if(base::xvchain_t::instance().get_xblockstore()->get_latest_connected_block(_vaddr,latest_connected_block_height,latest_connected_block_hash))
    {
        base::xauto_ptr<base::xvbstate_t> bstate = base::xvchain_t::instance().get_xstatestore()->get_blkstate_store()->get_block_state(_vaddr,latest_connected_block_height,latest_connected_block_hash, metrics::statestore_access_from_vnodesrv_load_state);
        if (bstate != nullptr) {
            xaccount_ptr_t account = std::make_shared<xunit_bstate_t>(bstate.get());
            return account;
        }
        xerror("xstore::query_account fail-load state directly.account=%s with height=%" PRId64 ",block_hash=%s", address.c_str(),latest_connected_block_height,xstring_utl::to_hex(latest_connected_block_hash).c_str());
    }
    else
    {
        ...
        xerror("xstore::query_account fail-load state.account=%s with height=%" PRId64 ",block_hash=%s", address.c_str(),_block->get_height(),xstring_utl::to_hex(_block->get_block_hash()).c_str());
    }
    return nullptr;
}
