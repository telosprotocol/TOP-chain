...
template <class T> struct Converter { static T convert(XRLP const&, int) {  } };
...
public:
    ...
    explicit XRLP(bytesConstRef _d, Strictness _s = VeryStrict);

    /// Construct a node of value given in the bytes.
    explicit XRLP(bytes const& _d, Strictness _s = VeryStrict): XRLP(&_d, _s) {}

    /// Construct a node to read XRLP data in the bytes given.
    XRLP(byte const* _b, unsigned _s, Strictness _st = VeryStrict): XRLP(bytesConstRef(_b, _s), _st) {}

    /// Construct a node to read XRLP data in the string.
    explicit XRLP(std::string const& _s, Strictness _st = VeryStrict): XRLP(bytesConstRef((byte const*)_s.data(), _s.size()), _st) {}

    /// The bare data of the XRLP.
    bytesConstRef data() const { return m_data; }

    /// @returns true if the XRLP is non-null.
    ...
    size_t itemCountStrict() const { if (!isList())    assert(true);  return items(); }

    /// @returns the number of bytes in the data, or zero if it isn't data.
    size_t size() const { return isData() ? length() : 0; }
    size_t sizeStrict() const { if (!isData()) assert(true); return length(); }
    ...
    /// Subscript operator.
    /// @returns the list item @a _i if isList() and @a _i < listItems(), or XRLP() otherwise.
    /// @note if used to access items in ascending order, this is efficient.
    XRLP operator[](size_t _i) const;

    using element_type = XRLP;

    /// @brief Iterator class for iterating through items of XRLP list.
    class iterator
    {
        friend class XRLP;

    public:
        using value_type = XRLP;
        using element_type = XRLP;

        iterator& operator++();
        iterator operator++(int) { auto ret = *this; operator++(); return ret; }
        XRLP operator*() const { return XRLP(m_currentItem); }
    ...
    private:
        iterator() {}
        iterator(XRLP const& _parent, bool _begin);
    ...
    std::string toString(int _flags = LaissezFaire) const { if (!isData()) { if (_flags & ThrowOnFail) assert(true); else return std::string(); } return payload().cropped(0, length()).toString(); }
    ...
    bytesConstRef payload() const { auto l = length(); if (l > m_data.size())    assert(true);  return m_data.cropped(payloadOffset(), l); }
...
private:
    ...
    /// @returns the size in bytes of the payload, as given by the XRLP as opposed to as inferred from m_data.
    size_t length() const;
    ...
    /// @returns the size encoded into the XRLP in @a _data and throws if _data is too short.
    static size_t sizeAsEncoded(bytesConstRef _data) { return XRLP(_data, ThrowOnFail | FailIfTooSmall).actualSize(); }
...
template <> struct Converter<std::string> { static std::string convert(XRLP const& _r, int _flags) { return _r.toString(_flags); } };
template <> struct Converter<bytes> { static bytes convert(XRLP const& _r, int _flags) { return _r.toBytes(_flags); } };
template <> struct Converter<uint8_t> { static uint8_t convert(XRLP const& _r, int _flags) { return _r.toInt<uint8_t>(_flags); } };
template <> struct Converter<uint16_t> { static uint16_t convert(XRLP const& _r, int _flags) { return _r.toInt<uint16_t>(_flags); } };
template <> struct Converter<uint32_t> { static uint32_t convert(XRLP const& _r, int _flags) { return _r.toInt<uint32_t>(_flags); } };
template <> struct Converter<uint64_t> { static uint64_t convert(XRLP const& _r, int _flags) { return _r.toInt<uint64_t>(_flags); } };
template <> struct Converter<u160> { static u160 convert(XRLP const& _r, int _flags) { return _r.toInt<u160>(_flags); } };
template <> struct Converter<u256> { static u256 convert(XRLP const& _r, int _flags) { return _r.toInt<u256>(_flags); } };
template <> struct Converter<bigint> { static bigint convert(XRLP const& _r, int _flags) { return _r.toInt<bigint>(_flags); } };
template <unsigned N> struct Converter<FixedHash<N>> { static FixedHash<N> convert(XRLP const& _r, int _flags) { return _r.toHash<FixedHash<N>>(_flags); } };
template <class T, class U> struct Converter<std::pair<T, U>> { static std::pair<T, U> convert(XRLP const& _r, int _flags) { return _r.toPair<T, U>(_flags); } };
template <class T> struct Converter<std::vector<T>> { static std::vector<T> convert(XRLP const& _r, int _flags) { return _r.toVector<T>(_flags); } };
template <class T> struct Converter<std::set<T>> { static std::set<T> convert(XRLP const& _r, int _flags) { return _r.toSet<T>(_flags); } };
template <class T> struct Converter<std::unordered_set<T>> { static std::unordered_set<T> convert(XRLP const& _r, int _flags) { return _r.toUnorderedSet<T>(_flags); } };
template <class T, size_t N> struct Converter<std::array<T, N>> { static std::array<T, N> convert(XRLP const& _r, int _flags) { return _r.toArray<T, N>(_flags); } };

template <class T> inline T XRLP::convert(int _flags) const { return Converter<T>::convert(*this, _flags); }

/**
 * @brief Class for writing to an XRLP bytestream.
 */
class RLPStream
{
public:
    ...
    /// Appends an arbitrary XRLP fragment - this *must* be a single item unless @a _itemCount is given.
    RLPStream& append(XRLP const& _rlp, size_t _itemCount = 1) { return appendRaw(_rlp.data(), _itemCount); }
    ...
    /// Appends raw (pre-serialised) XRLP data. Use with caution.
...
/// The empty string in XRLP format.
extern bytes RLPNull;

/// The empty list in XRLP format.
extern bytes RLPEmptyList;

/// Human readable version of XRLP.
std::ostream& operator<<(std::ostream& _out, XRLP const& _d);
