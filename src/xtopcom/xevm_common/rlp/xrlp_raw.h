...
std::tuple<xrlp_elem_kind, xspan_t<xbyte_t const>, xspan_t<xbyte_t const>> split(xspan_t<xbyte_t const> b, std::error_code & ec);
...
std::pair<xspan_t<xbyte_t const>, xspan_t<xbyte_t const>> split_string(xspan_t<xbyte_t const> b, std::error_code & ec);
...
std::pair<xspan_t<xbyte_t const>, xspan_t<xbyte_t const>> split_list(xspan_t<xbyte_t const> b, std::error_code & ec);
...
std::tuple<xrlp_elem_kind, uint64_t, uint64_t> read_kind(xspan_t<xbyte_t const> buf, std::error_code & ec);

uint64_t readSize(xbytes_t const & b, uint8_t slen, std::error_code & ec);
uint64_t read_size(xspan_t<xbyte_t const> b, uint8_t slen, std::error_code & ec);
