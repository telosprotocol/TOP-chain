...
public:
    ...
    virtual void PutWithPrefix(xbytes_t const & key, xbytes_t const & value, std::error_code & ec) = 0;
    virtual void Delete(xbytes_t const & key, std::error_code & ec) = 0;
    virtual void DeleteWithPrefix(xbytes_t const & key, std::error_code & ec) = 0;
};
...
public:
    ...
    virtual bool HasWithPrefix(xbytes_t const & key, std::error_code & ec) = 0;
    virtual xbytes_t Get(xbytes_t const & key, std::error_code & ec) = 0;
    virtual xbytes_t GetWithPrefix(xbytes_t const & key, std::error_code & ec) = 0;
};
...
inline void DeleteUnit(xkv_db_face_ptr_t db, xbytes_t const & unit_key) {
    std::error_code ec;
    db->DeleteWithPrefix(unit_key, ec);
    if (ec) {
