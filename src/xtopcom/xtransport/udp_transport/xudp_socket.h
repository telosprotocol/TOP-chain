...
protected:
    ...
    virtual int32_t recv(uint64_t from_xip_addr_low,
...
public:
    ...
    void SetStatus(enum_xudp_status s) {
        m_status = s;
    }
...
protected:
    ...
    int send_ping_packet(std::string target_ip_addr,
                         ...
                         uint16_t TTL);
    virtual int32_t on_ping_packet_recv(base::xpacket_t & packet_raw, int32_t cur_thread_id, uint64_t timenow_ms, xendpoint_t * from_child_end) override;
