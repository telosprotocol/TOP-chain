...
bool xp2pudp_t::on_endpoint_open(const int32_t error_code, const int32_t cur_thread_id, const uint64_t timenow_ms, xendpoint_t * from_child) {
    ...
    TOP_DBG_INFO(
        ...
        get_local_real_port(),
        get_local_logic_port(),
        get_local_logic_port_token(),
        get_peer_ip_address().c_str(),
        get_peer_real_port(),
        get_peer_logic_port(),
        get_peer_logic_port_token(),
        get_thread_id(),
        get_obj_id(),
        this,
        get_refcount(),
...
int32_t xp2pudp_t::recv(uint64_t from_xip_addr_low,
...
int XudpSocket::send_ping_packet(std::string target_ip_addr,
...
int32_t XudpSocket::on_ping_packet_recv(base::xpacket_t & packet_raw, int32_t cur_thread_id, uint64_t timenow_ms, xendpoint_t * from_child_end) {
...
int XudpSocket::SendPing(const xbyte_buffer_t & data, const std::string & peer_ip, uint16_t peer_port) {
    uint8_t local_buf[kUdpPacketBufferSize];
    base::xpacket_t packet(base::xcontext_t::instance(), local_buf, sizeof(local_buf), 0, 0, false);
    packet.get_body().push_back((uint8_t *)data.data(), data.size());  // NOLINT
    packet.set_to_ip_addr(peer_ip);
