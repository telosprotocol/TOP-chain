...
bool UdpTransport::Init(std::string const & local_ip, uint16_t local_port, MultiThreadHandler * message_handler) {
    ...
    TOP_INFO("UdpTransport::Init() success[%s:%d]", this->local_ip().c_str(), this->local_port());
    return true;
}
...
int UdpTransport::TransSendData(std::string const & data, const std::string & peer_ip, uint16_t peer_port, uint16_t priority_flag) {
    ...
    return udp_socket_->XudpSendData(data, peer_ip, peer_port, priority_flag);
}
