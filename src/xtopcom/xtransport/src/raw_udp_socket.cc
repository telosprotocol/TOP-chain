...
int RawUdpSocket::SendData(base::xpacket_t& packet) {
    ...
    XMETRICS_COUNTER_INCREMENT("transport_packet_send", 1);
    XMETRICS_COUNTER_INCREMENT("transport_bandwidth_send", packet.get_body().size());
...
int RawUdpSocket::RegisterNodeCallback(std::function<int32_t(std::string const& node_addr, std::string const& node_sign)> cb) {
	return kTransportSuccess;
}
int RawUdpSocket::CheckRatelimitMap(const std::string& to_addr) {
	return kTransportSuccess;
}
