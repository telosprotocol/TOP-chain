...
impl rlp::Decodable for PublicKeyBytes {
    fn decode(rlp: &Rlp) -> Result<Self, DecoderError> {
        let data: String = rlp.val_at(0).unwrap();
        let mut pb: PublicKeyBytes = PublicKeyBytes {
            0: [0; PUBLIC_KEY_BYTES_LEN],
        };
        pb.0.copy_from_slice(&data.as_bytes()[0..PUBLIC_KEY_BYTES_LEN]);
        Ok(pb)
    }
}

#[derive(Debug, Clone, BorshDeserialize, BorshSerialize, tree_hash_derive::TreeHash, rlp_derive::RlpDecodable)]
...
#[derive(Debug, Clone, BorshDeserialize, BorshSerialize, rlp_derive::RlpDecodable)]
#[cfg_attr(not(target_arch = "wasm32"), derive(Serialize, Deserialize))]
