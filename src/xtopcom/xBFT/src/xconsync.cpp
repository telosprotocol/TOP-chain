...
namespace top
{
    namespace xconsensus
    {
        ...
        int   xBFTSyncdrv::handle_sync_request_msg(const xvip2_t & from_addr,const xvip2_t & to_addr,xcspdu_fire * event_obj,int32_t cur_thread_id,uint64_t timenow_ms,xcsobject_t * _parent)
        {
            ...
            if(NULL != _local_block)
            {
                ...
                if (sync_targets & enum_xsync_target_block_output_offdata) {
                    if(  (_local_block->should_has_output_offdata()) //link offdata data
                       &&(!_local_block->has_output_offdata()) ) //but dont have offdata avaiable now
                    {
                        //_local_block need reload output offdata
                        get_vblockstore()->load_block_output_offdata(*this, _local_block);
                        if (!_local_block->has_output_offdata()) {
        ...
        int   xBFTSyncdrv::handle_sync_respond_msg(const xvip2_t & from_addr,const xvip2_t & to_addr,xcspdu_fire * event_obj,int32_t cur_thread_id,uint64_t timenow_ms,xcsobject_t * _parent)
        {
            ...
            if(_sync_block->set_input_data(input_resource) == false)//not match input hash  in header
            {
            ...
            if(_sync_block->set_output_data(output_resource) == false)//not match output hash in cert
            {
