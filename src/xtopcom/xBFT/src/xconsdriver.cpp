    ...
    namespace xconsensus
    {
        ...
        int  xBFTdriver_t::handle_proposal_msg(const xvip2_t & from_addr,const xvip2_t & to_addr,xcspdu_fire * event_obj,int32_t cur_thread_id,uint64_t timenow_ms,xcsobject_t * _parent)
        {
            ...
            if (event_obj->_packet.get_msg_type() == enum_consensus_msg_type_proposal) {
                ...
                block_object_data = _proposal_msg.get_block_header_and_cert();
                input_proposal = _proposal_msg.get_input_proposal();
                expired_ms = _proposal_msg.get_expired_ms();
            }
            ...
            if (event_obj->_packet.get_msg_type() == enum_consensus_msg_type_proposal) {
                block = base::xvblock_t::create_block_object(block_object_data);
            } else {
                block = base::xvblock_t::create_block_object_header_and_qcert(block_object_data);
            }
            base::xauto_ptr<base::xvblock_t> _peer_block(block);
