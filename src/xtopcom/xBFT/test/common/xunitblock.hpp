...
namespace top
{
    namespace test
    {
        ...
        public: //return raw ptr with added reference,caller respond to release it after that.
            ...
            virtual base::xauto_ptr<base::xvblock_t>  get_genesis_block(const std::string & account,const int atag = 0) ;//genesis block
            virtual base::xauto_ptr<base::xvblock_t>  get_latest_cert_block(const std::string & account,const int atag = 0);//highest view# for any status
            virtual base::xauto_ptr<base::xvblock_t>  get_latest_locked_block(const std::string & account,const int atag = 0)    ;//block with locked status
            virtual base::xauto_ptr<base::xvblock_t>  get_latest_committed_block(const std::string & account,const int atag = 0) ;//block with committed status
            virtual base::xauto_ptr<base::xvblock_t>  get_latest_executed_block(const std::string & account,const int atag = 0)  ;//block with executed status
            virtual base::xauto_ptr<base::xvblock_t>  get_latest_connected_block(const std::string & account,const int atag = 0) ;//block connected to genesis
            virtual base::xauto_ptr<base::xvblock_t>  get_latest_full_block(const std::string & account,const int atag = 0)  ;//block has full state,genesis is a full block
            virtual base::xauto_ptr<base::xvblock_t>  load_block_object(const std::string & account,const uint64_t height,bool ask_full_load,const int atag = 0);
            
            virtual bool                load_block_input(base::xvblock_t* block,const int atag = 0)  ;//load and assign input data into block
            virtual bool                load_block_output(base::xvblock_t* block,const int atag = 0) ;//load and assign output data into block
            
            virtual bool                store_block(base::xvblock_t* block,const int atag = 0)  ; //return false if fail to store
            virtual bool                delete_block(base::xvblock_t* block,const int atag = 0) ; //return false if fail to delete
