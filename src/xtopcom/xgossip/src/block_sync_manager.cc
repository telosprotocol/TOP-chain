...
bool BlockSyncManager::CheckSyncFilterMap(const std::string& header_hash, const std::string& node_id) {
    {
        ...
        if (ifind == sync_ask_filter_map_.end()) {
            ...
            TOP_DEBUG("sync_ask_filter_map first insert header_hash:%s node:%s",
        ...
        for (auto& item : ifind->second) {
            if (item->node_id == node_id) {
                if (item->ask_count >= kSyncAskMaxCount) {
                    TOP_DEBUG("sync_ask_filter_map beyond askmax header_hash:%s node:%s",
                            HexEncode(header_hash).c_str(),
                            HexEncode(node_id).c_str());
                    return false;
                }
                ...
                TOP_DEBUG("sync_ask_filter_map update header_hash:%s node:%s",
                        HexEncode(header_hash).c_str(),
                        HexEncode(node_id).c_str());
                return true;
            }
        ...
        TOP_DEBUG("sync_ask_filter_map update push header_hash:%s node:%s",
                HexEncode(header_hash).c_str(),
                HexEncode(node_id).c_str());
        return true;
    }
...
void BlockSyncManager::HandleSyncResponse(
    ...
    // call callback
    if (sync_message.type() == kElectVhostRumorP2PMessage
            || sync_message.type() == kElectVhostRumorGossipMessage) {
