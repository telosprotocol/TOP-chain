...
class TestGossipSetLayer : public testing::Test {
public:
    static void SetUpTestCase() {
        ...
        bloom_gossip_set_layer_ptr_ = std::make_shared<GossipSetLayer>(core_transport);
    ...
    virtual void SetUp() {
        ASSERT_TRUE(bloom_gossip_set_layer_ptr_);
        ASSERT_TRUE(gossip_root_routing_table_ptr_);
        ASSERT_TRUE(gossip_routing_table_ptr_);
    }
    ...
    static  GossipInterfacePtr bloom_gossip_set_layer_ptr_;
    static kadmlia::RoutingTablePtr gossip_routing_table_ptr_;
    static kadmlia::RoutingTablePtr gossip_root_routing_table_ptr_;

};

GossipInterfacePtr TestGossipSetLayer::bloom_gossip_set_layer_ptr_ =  nullptr;
kadmlia::RoutingTablePtr TestGossipSetLayer::gossip_routing_table_ptr_ = nullptr;
kadmlia::RoutingTablePtr TestGossipSetLayer::gossip_root_routing_table_ptr_ = nullptr;

kadmlia::RoutingTablePtr TestGossipSetLayer::CreateRoutingTable(
        transport::TransportPtr transport,
        uint32_t routing_max_node,
        uint32_t xnetwork_id,
        uint8_t zone_id,
...
kadmlia::RoutingTablePtr TestGossipSetLayer::CreateRootRoutingTable(
        transport::TransportPtr transport,
        uint32_t size,
        const base::Config& config) {
...
void TestGossipSetLayer::CreateMessage(transport::protobuf::RoutingMessage& message, const std::string& src, const std::string& des) {
    return CreateMessageForTest(message, src, des);
}

TEST_F(TestGossipSetLayer, Broadcast) {
    ...
    bloom_gossip_set_layer_ptr_->Broadcast(RandomUint64(), message, ptr_neighbors);
}

TEST_F(TestGossipSetLayer, Broadcast_maxhop) {
    auto ptr_neighbors = gossip_routing_table_ptr_->GetUnLockNodes();
    ASSERT_TRUE(ptr_neighbors);
    ASSERT_EQ(ptr_neighbors->size(), gossip_routing_table_ptr_->nodes_size());
...
TEST_F(TestGossipSetLayer, Broadcast_evail) {
    auto ptr_neighbors = gossip_routing_table_ptr_->GetUnLockNodes();
    ASSERT_TRUE(ptr_neighbors);
    ASSERT_EQ(ptr_neighbors->size(), gossip_routing_table_ptr_->nodes_size());
...
TEST_F(TestGossipSetLayer, Broadcast_invalid_hash64) {
    auto ptr_neighbors = gossip_routing_table_ptr_->GetUnLockNodes();
    ASSERT_TRUE(ptr_neighbors);
    ASSERT_EQ(ptr_neighbors->size(), gossip_routing_table_ptr_->nodes_size());
...
/*
TEST_F(TestGossipSetLayer, BroadcastWithNoFilter) {
    auto ptr_neighbors = gossip_routing_table_ptr_->GetUnLockNodes();
    ASSERT_TRUE(ptr_neighbors);
    ASSERT_EQ(ptr_neighbors->size(), gossip_routing_table_ptr_->nodes_size());
    ...
    dynamic_cast<GossipSetLayer*>(bloom_gossip_set_layer_ptr_.get())->BroadcastWithNoFilter(RandomString(36), message, *ptr_neighbors);
}

TEST_F(TestGossipSetLayer, BroadcastWithNoFilter_evail) {
    auto ptr_neighbors = gossip_routing_table_ptr_->GetUnLockNodes();
    ASSERT_TRUE(ptr_neighbors);
    ASSERT_EQ(ptr_neighbors->size(), gossip_routing_table_ptr_->nodes_size());
...
TEST_F(TestGossipSetLayer, BroadcastWithNoFilter_maxhop) {
    auto ptr_neighbors = gossip_routing_table_ptr_->GetUnLockNodes();
    ASSERT_TRUE(ptr_neighbors);
    ASSERT_EQ(ptr_neighbors->size(), gossip_routing_table_ptr_->nodes_size());
