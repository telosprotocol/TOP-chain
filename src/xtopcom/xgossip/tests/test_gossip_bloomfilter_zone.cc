...
class TestGossipBloomfilterZone: public testing::Test {
public:
    static void SetUpTestCase() {
        ...
        bloom_gossip_zone_ptr_ = std::make_shared<GossipBloomfilterZone>(core_transport);
    ...
    virtual void SetUp() {
        ASSERT_TRUE(bloom_gossip_zone_ptr_);
        ASSERT_TRUE(gossip_root_routing_table_ptr_);
        ASSERT_TRUE(gossip_routing_table_ptr_);
    }
    ...
    static  GossipInterfacePtr bloom_gossip_zone_ptr_;
    static kadmlia::RoutingTablePtr gossip_routing_table_ptr_;
    static kadmlia::RoutingTablePtr gossip_root_routing_table_ptr_;

};

GossipInterfacePtr TestGossipBloomfilterZone::bloom_gossip_zone_ptr_ =  nullptr;
kadmlia::RoutingTablePtr TestGossipBloomfilterZone::gossip_routing_table_ptr_ = nullptr;
kadmlia::RoutingTablePtr TestGossipBloomfilterZone::gossip_root_routing_table_ptr_ = nullptr;

kadmlia::RoutingTablePtr TestGossipBloomfilterZone::CreateRoutingTable(
        transport::TransportPtr transport,
        uint32_t routing_max_node,
        uint32_t xnetwork_id,
        uint8_t zone_id,
...
kadmlia::RoutingTablePtr TestGossipBloomfilterZone::CreateRootRoutingTable(
        transport::TransportPtr transport,
        uint32_t size,
        const base::Config& config) {
...
void TestGossipBloomfilterZone::CreateMessage(transport::protobuf::RoutingMessage& message, const std::string& src, const std::string& des) {
    CreateMessageForTest(message, src, des);
    message.set_is_root(true);
}

TEST_F(TestGossipBloomfilterZone, Broadcast_zone4) {
    ...
    bloom_gossip_zone_ptr_->Broadcast(RandomUint64(), message, new_ptr_neighbors);
    bloom_gossip_zone_ptr_->Broadcast(RandomUint64(), message, new_ptr_neighbors);
}

TEST_F(TestGossipBloomfilterZone, Broadcast_zone3_try) {
    auto ptr_neighbors = gossip_root_routing_table_ptr_->GetUnLockNodes();
    ASSERT_TRUE(ptr_neighbors);
    ASSERT_EQ(ptr_neighbors->size(), gossip_root_routing_table_ptr_->nodes_size());
    ...
    for (uint32_t t = 0; t < 100; ++t) {
        ...
        bloom_gossip_zone_ptr_->Broadcast(RandomUint64(), t_message, new_ptr_neighbors);
        bloom_gossip_zone_ptr_->Broadcast(RandomUint64(), t_message, new_ptr_neighbors);
...
TEST_F(TestGossipBloomfilterZone, Broadcast_maxhop) {
    auto ptr_neighbors = gossip_root_routing_table_ptr_->GetUnLockNodes();
    ASSERT_TRUE(ptr_neighbors);
    ASSERT_EQ(ptr_neighbors->size(), gossip_root_routing_table_ptr_->nodes_size());
    ...
    bloom_gossip_zone_ptr_->Broadcast(RandomUint64(), message, ptr_neighbors);
    bloom_gossip_zone_ptr_->Broadcast(RandomUint64(), message, ptr_neighbors);
}

TEST_F(TestGossipBloomfilterZone, Broadcast_evail) {
    auto ptr_neighbors = gossip_root_routing_table_ptr_->GetUnLockNodes();
    ASSERT_TRUE(ptr_neighbors);
    ASSERT_EQ(ptr_neighbors->size(), gossip_root_routing_table_ptr_->nodes_size());
...
TEST_F(TestGossipBloomfilterZone, Broadcast_invalid_hash64) {
    auto ptr_neighbors = gossip_root_routing_table_ptr_->GetUnLockNodes();
    ASSERT_TRUE(ptr_neighbors);
    ASSERT_EQ(ptr_neighbors->size(), gossip_root_routing_table_ptr_->nodes_size());
...
TEST_F(TestGossipBloomfilterZone, BroadcastWithNoFilter) {
    auto ptr_neighbors = gossip_root_routing_table_ptr_->GetUnLockNodes();
    ASSERT_TRUE(ptr_neighbors);
    ASSERT_EQ(ptr_neighbors->size(), gossip_root_routing_table_ptr_->nodes_size());
    ...
    dynamic_cast<GossipBloomfilterZone*>(bloom_gossip_zone_ptr_.get())->BroadcastWithNoFilter(RandomString(36), message, *ptr_neighbors);
}

TEST_F(TestGossipBloomfilterZone, BroadcastWithNoFilter_evail) {
    auto ptr_neighbors = gossip_root_routing_table_ptr_->GetUnLockNodes();
    ASSERT_TRUE(ptr_neighbors);
    ASSERT_EQ(ptr_neighbors->size(), gossip_root_routing_table_ptr_->nodes_size());
...
TEST_F(TestGossipBloomfilterZone, BroadcastWithNoFilter_maxhop) {
    auto ptr_neighbors = gossip_root_routing_table_ptr_->GetUnLockNodes();
    ASSERT_TRUE(ptr_neighbors);
    ASSERT_EQ(ptr_neighbors->size(), gossip_root_routing_table_ptr_->nodes_size());
