...
public:
    static void SetUpTestCase() {
        ...
        bloom_gossip_layer_ptr_ = std::make_shared<GossipBloomfilterLayer>(core_transport);
...
GossipInterfacePtr TestGossipBloomfilterLayer::bloom_gossip_layer_ptr_ =  nullptr;
kadmlia::RoutingTablePtr TestGossipBloomfilterLayer::gossip_routing_table_ptr_ = nullptr;
kadmlia::RoutingTablePtr TestGossipBloomfilterLayer::gossip_root_routing_table_ptr_ = nullptr;


kadmlia::RoutingTablePtr TestGossipBloomfilterLayer::CreateRoutingTable(
        transport::TransportPtr transport,
        uint32_t routing_max_node,
        uint32_t xnetwork_id,
        uint8_t zone_id,
...
kadmlia::RoutingTablePtr TestGossipBloomfilterLayer::CreateRootRoutingTable(
        transport::TransportPtr transport,
        uint32_t size,
        const base::Config& config) {
...
void TestGossipBloomfilterLayer::CreateMessage(transport::protobuf::RoutingMessage& message, const std::string& src, const std::string& des) {
    return CreateMessageForTest(message, src, des);
}

// not important
TEST_F(TestGossipBloomfilterLayer, Broadcast_empty_body) {
...
TEST_F(TestGossipBloomfilterLayer, Broadcast_maxhop) {
    transport::protobuf::RoutingMessage message;
...
TEST_F(TestGossipBloomfilterLayer, Broadcast_largehop) {
    transport::protobuf::RoutingMessage message;
    auto src = gossip_routing_table_ptr_->get_local_node_info()->id();
    ASSERT_FALSE(src.empty());
    auto dst = gossip_routing_table_ptr_->GetRandomNode()->node_id;
...
TEST_F(TestGossipBloomfilterLayer, Broadcast_evail) {
    transport::protobuf::RoutingMessage message;
    auto src = gossip_routing_table_ptr_->get_local_node_info()->id();
    ASSERT_FALSE(src.empty());
    auto dst = gossip_routing_table_ptr_->GetRandomNode()->node_id;
...
TEST_F(TestGossipBloomfilterLayer, Broadcast_invalid_hash64) {
}
