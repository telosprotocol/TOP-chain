...
std::shared_ptr<xunit_service::xblock_maker_face> xblockmaker_factory::create_table_proposal(const observer_ptr<base::xvblockstore_t> & blockstore,
                                                                                            const observer_ptr<xtxpool_v2::xtxpool_face_t> & txpool,
                                                                                            const observer_ptr<mbus::xmessage_bus_face_t> & bus) {
    xblockmaker_resources_ptr_t resources = std::make_shared<xblockmaker_resources_impl_t>(blockstore, txpool, bus);
