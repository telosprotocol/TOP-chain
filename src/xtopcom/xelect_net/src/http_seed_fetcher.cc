...
HttpSeedFetcher::HttpSeedFetcher(std::string const & url) : base_t{url} {
}

bool HttpSeedFetcher::GetSeeds(std::vector<std::string> & url_seeds) {
    std::string result;
    try {
        result = request_get();
...
bool HttpSeedFetcher::GetEdgeSeeds(std::vector<std::string> & url_seeds) {
    std::string result;
    try {
        result = request_get();
        xinfo("[seeds] get result:%s", result.c_str());
    ...
    try {
        ...
        xwarn("[edgeseeds] json parse error:%s", e.what());
        return false;
    }
    return true;
}
