...
void xtop_gasfee::init(std::error_code & ec) {
    ...
    do {
        if (max_gasfee > 0) {
            ...
            if (eth_max_gasfee_256 > UINT64_MAX) {
                ...
                xwarn("[xtop_gasfee::init] tx_limited_gasfee_exceeded: %s", eth_max_gasfee_256.str().c_str());
                return;
            }
    ...
    xdbg("[xtop_gasfee::init] final, m_free_tgas: %lu, m_converted_tgas: %lu, m_eth_converted_tgas: %lu", m_free_tgas, m_converted_tgas, m_eth_converted_tgas);
}
...
void xtop_gasfee::store_abnormal() {
    ...
    m_detail.m_state_last_time = m_time;
    auto top_usage = tgas_to_balance(m_converted_tgas);
    m_detail.m_state_burn_balance = top_usage;
    m_detail.m_tx_used_deposit = top_usage;
