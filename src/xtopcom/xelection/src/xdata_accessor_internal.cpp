...
xtop_data_accessor::group_element(common::xnetwork_id_t const & network_id,
    ...
    if (version.empty()) {
        return cluster_element->group_element(group_id, m_logic_timer->logic_time(), ec);
    } else {
...
xtop_data_accessor::group_element(common::xnetwork_id_t const & network_id,
    ...
    if (network_version.empty()) {
        return cluster_element->group_element(group_id, m_logic_timer->logic_time(), ec);
    } else {
        auto group_element = cluster_element->group_element(group_id, network_version, ec);
        if (ec == xdata_accessor_errc_t::group_not_exist && network_version == common::xdefault_network_version) {
            ec = xdata_accessor_errc_t::success;
            group_element = cluster_element->group_element(group_id, m_logic_timer->logic_time(), ec);
        }
...
xtop_data_accessor::group_element(common::xnetwork_id_t const & network_id,
    ...
    return cluster_element->group_element(group_id, logic_time, ec);
