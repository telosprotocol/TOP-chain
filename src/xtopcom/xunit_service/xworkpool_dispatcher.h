...
public:
    ...
    virtual bool subscribe(const std::vector<base::xtableid_t> & tables, const xvip2_t & xip, const common::xlogic_time_t& start_time);
...
protected:
    std::string       account(base::xtableid_t & tableid);
    base::xtableid_t  get_tableid(const std::string & account);
    int16_t           get_thread_index(base::xworkerpool_t * pool, base::xtableid_t& tableid);
    base::xworker_t * get_worker(base::xworkerpool_t * pool, base::xtableid_t& table_id);
