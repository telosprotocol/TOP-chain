...
int16_t xworkpool_dispatcher::get_thread_index(base::xworkerpool_t * pool, base::xtableid_t& table_id) {
...
std::string xworkpool_dispatcher::account(base::xtableid_t & tableid) {
    return data::xblocktool_t::make_address_table_account(tableid.get_zone_index(), tableid.get_subaddr());
}

base::xtableid_t xworkpool_dispatcher::get_tableid(const std::string & account) {
...
bool xworkpool_dispatcher::start(const xvip2_t & xip, const common::xlogic_time_t& start_time) {
    ...
    if (elect_face != nullptr) {
        std::vector<base::xtableid_t> tables;
...
bool xworkpool_dispatcher::subscribe(const std::vector<base::xtableid_t> & tables, const xvip2_t & xip, const common::xlogic_time_t& start_time) {
...
base::xworker_t * xworkpool_dispatcher::get_worker(base::xworkerpool_t * pool, base::xtableid_t & table_id) {
    auto pool_index = get_thread_index(pool, table_id);
//    auto pool_thread_ids = pool->get_thread_ids();
