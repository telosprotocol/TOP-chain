...
public:
    ...
    using hash_result_type = typename xhashable_t<xtop_simple_id<TagT, IdT>>::hash_result_type;
...
public:
    ...
    xtop_simple_id
    max() noexcept {
        return xtop_simple_id{ std::numeric_limits<value_type>::max() };
    }
    ...
    xtop_simple_id
    min() noexcept {
        return xtop_simple_id{ std::numeric_limits<value_type>::min() };
    }
...
template <typename TagT, typename IdT = std::uint64_t>
using xsimple_id_t = xtop_simple_id<TagT, IdT>;
...
template <typename TagT, typename IdT>
struct hash<top::xsimple_id_t<TagT, IdT>> final
{
    std::size_t
    operator()(top::xsimple_id_t<TagT, IdT> const & id) const noexcept {
        return static_cast<std::size_t>(id.hash());
    }
};

template <typename TagT, typename IdT>
struct numeric_limits<top::xsimple_id_t<TagT, IdT>> final
{
    static
    constexpr
    top::xsimple_id_t<TagT, IdT>
    min() noexcept {
        return top::xsimple_id_t<TagT, IdT>::min();
    }
    ...
    top::xsimple_id_t<TagT, IdT>
    max() noexcept {
        return top::xsimple_id_t<TagT, IdT>::max();
    }
};
...
template <typename TagT, typename IdT>
std::ostream &
operator<<(std::ostream & o, top::xsimple_id_t<TagT, IdT> const & id) {
    o << id.to_string();
    return o;
}
