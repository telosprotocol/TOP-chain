...
auto empty_unclehash = static_cast<h256>(from_hex("1dcc4de8dec75d7aab85b567b6ccd41ad312451b948a7413f0a142fd40d49347"));
...
bool xtop_evm_heco_client_contract::init(const xbytes_t & rlp_bytes, state_ptr state) {
    ...
    for (size_t i = 0; i < headers.size(); ++i) {
        ...
        xheco_snap_info_t snap_info{snap_hash, h.parent_hash, h.number};
...
bool xtop_evm_heco_client_contract::verify(const xeth_header_t & prev_header, const xeth_header_t & new_header, xheco_snapshot_t & snap, state_ptr state) const {
    ...
    if (new_header.gas_limit > max_gas_limit) {
        xwarn("[xtop_evm_heco_client_contract::verify] gaslimit too big: %lu > %lu", new_header.gas_limit, max_gas_limit);
        return false;
    }
...
bool xtop_evm_heco_client_contract::record(const xeth_header_t & header, const xheco_snapshot_t & snap, state_ptr state) {
...
bool xtop_evm_heco_client_contract::rebuild(const xeth_header_t & header, const xheco_snap_info_t & last_info, const xheco_snap_info_t & cur_info, state_ptr state) {
...
bool xtop_evm_heco_client_contract::get_snap_info(const h256 hash, xheco_snap_info_t & header_info, state_ptr state) const {
    auto k = hash.asBytes();
...
bool xtop_evm_heco_client_contract::set_snap_info(const h256 hash, const xheco_snap_info_t & snap_info, state_ptr state) {
    auto k = hash.asBytes();
