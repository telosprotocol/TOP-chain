...
data::xblock_ptr_t extract_block_from(xevent_store_block_to_db_ptr_t const & ev) noexcept {
    ...
    XMETRICS_GAUGE(metrics::blockstore_access_from_mbus, 1);
    xobject_ptr_t<base::xvblock_t> blk = blkstore->load_block_object(base::xvaccount_t{ ev->owner }, ev->blk_height, ev->blk_hash, false);
    xassert(blk != nullptr);
