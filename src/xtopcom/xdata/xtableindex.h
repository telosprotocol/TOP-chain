...
class xaccount_index_t {
 ...
 public:
    ...
    uint64_t                get_latest_unit_clock() const {return m_latest_unit_clock;}
    uint32_t                get_latest_unit_hash32() const {return m_latest_unit_hash;}
 ...
 public:
    ...
    void                    set_latest_unit_clock(uint64_t _clock) {m_latest_unit_clock = _clock;}
    void                    set_latest_unit_hash(const std::string & hash);

 private:
    uint64_t        m_latest_unit_height{0};
    uint64_t        m_latest_unit_clock{0};
    uint32_t        m_latest_unit_hash{0};
...
class xtable_mbt_binlog_t : public base::xdataunit_t {
 ...
 public:
    ...
    bool                                                get_unit_index(const std::string & account, xaccount_index_t & index) const;
...
class xtable_mbt_bucket_node_t : public base::xdataunit_t {
 ...
 public:
    void                    set_unit_index(const std::string & account, xaccount_index_t index);
    bool                    check_bucket() const;    // check if bucket is correct
    std::string             build_bucket_hash();
 public:
    bool                    get_unit_index(const std::string & account, xaccount_index_t & index) const;
    size_t                  get_account_size() const {return m_unit_indexs.size();}
    const std::string &     get_bucket_hash() const {return m_bucket_hash;}
 private:
    std::string             calc_bucket_hash() const;
 private:
    std::string                               m_bucket_hash;
    std::map<std::string, xaccount_index_t>   m_unit_indexs;
};
...
class xtable_mbt_t : public xbase_dataunit_t<xtable_mbt_t, xdata_type_table_mbt> {
 ...
 private:
    void            set_unit_index(const std::string & account, const xaccount_index_t & info);
