...
v1::xstandby_network_storage_result_t xtop_standby_network_storage_result::v1() const {
    v1::xstandby_network_storage_result_t r;
    r.set_activate_state(activated_state());

    std::transform(
        std::begin(m_results), std::end(m_results), std::inserter(r, std::end(r)), [](value_type const & input) -> v1::xstandby_network_storage_result_t::value_type {
