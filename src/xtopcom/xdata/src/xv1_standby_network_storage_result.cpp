...
xstandby_network_result_t xtop_standby_network_storage_result::network_result(common::xnode_type_t const node_type) const {
    ...
    for (auto const & p : m_results) {
        ...
        auto const & standby_node_info = top::get<election::v1::xstandby_node_info_t>(p);
...
v0::xstandby_network_storage_result_t xtop_standby_network_storage_result::v0() const {
    v0::xstandby_network_storage_result_t r;
    r.set_activate_state(activated_state());

    std::transform(
        std::begin(m_results), std::end(m_results), std::inserter(r, std::end(r)), [](value_type const & input) -> v0::xstandby_network_storage_result_t::value_type {
