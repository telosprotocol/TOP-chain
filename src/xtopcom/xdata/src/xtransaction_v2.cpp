...
int32_t xtransaction_v2_t::do_write_without_hash_signature(base::xstream_t & out) const {
    ...
    std::string compact_source_account = xvaccount_t::compact_address_to(m_source_account);
    // std::cout << "new " << m_source_account << " size:" << compact_source_account.size() << std::endl;
    out << compact_source_account;
    std::string compact_target_account = xvaccount_t::compact_address_to(m_target_account);
...
int32_t xtransaction_v2_t::do_read_without_hash_signature(base::xstream_t & in) {
    ...
    m_source_account = xvaccount_t::compact_address_from(compact_source_account);
    std::string compact_target_account;
    in >> compact_target_account;
    m_target_account = xvaccount_t::compact_address_from(compact_target_account);
...
void xtransaction_v2_t::adjust_target_address(uint32_t table_id) {
    if (m_adjust_target_addr.empty()) {
        m_adjust_target_addr = make_address_by_prefix_and_subaddr(m_target_account, table_id).value();
        xdbg("xtransaction_v2_t::adjust_target_address hash=%s,origin_addr=%s,new_addr=%s",
            get_digest_hex_str().c_str(), m_target_account.c_str(), m_adjust_target_addr.c_str());        
    }
    // else {
    //     xassert(false);
    // }
...
int32_t xtransaction_v2_t::set_different_source_target_address(const std::string & src_addr, const std::string & dst_addr) {
    m_source_account = src_addr;
    m_target_account = dst_addr;
    return xsuccess;
}
...
bool xtransaction_v2_t::sign_check() const {
    ...
    // xdbg("wish m_authorization %s, txhash:%s", to_hex_str(m_authorization).c_str(), get_digest_hex_str().c_str());
...
void xtransaction_v2_t::parse_to_json(xJson::Value& result_json) const {
    ...
    result_json["source_account"] = m_source_account;
    result_json["target_account"] = m_target_account;
    ...
    result_json["source_action_para"] = m_source_action_para;
    result_json["target_action_name"] = m_target_action_name;
    result_json["target_action_para"] = m_target_action_para;
...
void xtransaction_v2_t::construct_from_json(xJson::Value& request) {
    ...
    if (from.empty()) {
        // xdbg("wish construct from v2 struct");
        ...
        m_source_action_para = request["source_action_para"].asString();
        m_target_action_name = request["target_action_name"].asString();
        m_target_action_para = request["target_action_para"].asString();
        set_amount(request["amount"].asUInt64());
        m_token_name = request["token_name"].asString();
    } else {
        // xdbg("wish construct from v1 struct");
...
int32_t xtransaction_v2_t::parse(enum_xaction_type source_type, enum_xaction_type target_type, xtx_parse_data_t & tx_parse_data) {
    ...
    if (target_type == xaction_type_create_user_account) {
        tx_parse_data.m_new_account = m_source_account;
    }
#endif
    ...
    if (target_type == xaction_type_run_contract) {
        ...
        // xdbg("wish v2 target_action_name:%s", tx_parse_data.m_function_name.c_str());
    }
    ...
    if (target_type == xaction_type_pledge_token) {
        ...
        // xdbg("wish v2 pledge amount:%llu", tx_parse_data.m_asset.m_amount);
    }
    ...
    if (target_type == xaction_type_redeem_token) {
        ...
        stream >> tx_parse_data.m_asset.m_token_name;
        stream >> tx_parse_data.m_asset.m_amount;
        // xdbg("wish v2 redeem amount:%llu", tx_parse_data.m_asset.m_amount);
    }
    ...
    if (target_type == xaction_type_pledge_token_vote) {
        ...
        // xdbg("wish v2 pledge vote amount:%llu, %d", tx_parse_data.m_vote_num, tx_parse_data.m_lock_duration);
    }

    if (target_type == xaction_type_redeem_token_vote) {
        base::xstream_t stream(base::xcontext_t::instance(), (uint8_t *)m_target_action_para.data(), m_target_action_para.size());
        stream >> tx_parse_data.m_vote_num;
        // xdbg("wish v2 redeem vote amount:%llu", tx_parse_data.m_vote_num);
    }
