...
xtop_election_result::iterator xtop_election_result::insert(const_iterator hint, value_type const & v) {
    return m_cluster_results.insert(hint, v);
}

xtop_election_result::iterator xtop_election_result::insert(const_iterator hint, value_type && v) {
    return m_cluster_results.insert(hint, std::move(v));
}
...
v0::xelection_result_t xtop_election_result::v0() const {
    v0::xelection_result_t r;

    std::transform(
        std::begin(m_cluster_results), std::end(m_cluster_results), std::inserter(r, std::end(r)), [](value_type const & input) -> v0::xelection_result_t::value_type {
