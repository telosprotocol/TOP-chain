...
base::xtableid_t account_map_to_table_id(common::xaccount_address_t const & addr) {
    const std::string & account = addr.value();
    base::xvaccount_t _vaccount(account);
    return _vaccount.get_tableid();
}

std::string account_address_to_block_address(common::xaccount_address_t const & addr) {
    base::xtableid_t tableid = account_map_to_table_id(addr);
