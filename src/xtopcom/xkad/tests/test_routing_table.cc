...
public:
    static void SetUpTestCase() {
        ...
        {
            std::unique_lock<std::mutex> lock(routing_table_ptr_->bootstrap_nodes_mutex_);
            ASSERT_FALSE(routing_table_ptr_->bootstrap_nodes_.empty());
        }
    }
...
TEST_F(TestRoutingTable, FindClosestNodes) {
    ...
    routing_table_ptr_->FindClosestNodes(10, find_nodes);
}
...
TEST_F(TestRoutingTable, ClosestToTarget) {
    ...
    {
        RoutingTable::NodesLock lock(routing_table_ptr_->nodes_mutex_);
