...
bool MyRoutingTable::Init(const std::string& name_prefix, const std::string& supernode_ip, uint16_t supernode_port) {
    ...
    {
        using namespace std::placeholders;
        auto cb = std::bind(&MyRoutingTable::OnCommandGetrt3, this, _1);
        BenchCommand::Instance()->RegisterCommand("getrt3", cb);
    }
    {
        using namespace std::placeholders;
        auto cb = std::bind(&MyRoutingTable::OnCommandGetrt4, this, _1);
        BenchCommand::Instance()->RegisterCommand("getrt4", cb);
    }
    {
        using namespace std::placeholders;
...
void MyRoutingTable::OnCommandGetrt3(const BenchCommand::Arguments& args) {
    std::unique_lock<std::mutex> lock2(all_nodes_mutex_);

    int node_base = 0;
    int node_count = 1;
    if (args.size() >= 1) {
        node_base = ParseArg(args[0], 0);
    }
    if (args.size() >= 2) {
        node_count = ParseArg(args[1], 1);
    }

    // locate the first node
    int n = 0;
    auto it = all_nodes_map_.begin();
    while (it != all_nodes_map_.end() && n < node_base) {
        ++it;
        ++n;
    }

    // locate the later nodes
    std::vector<NodeInfoPtr> nodes;
    while (it != all_nodes_map_.end() && n < node_base + node_count) {
        nodes.push_back(it->second);
        ++it;
        ++n;
    }
...
void MyRoutingTable::OnCommandGetrt4(const BenchCommand::Arguments& args) {
    if (args.size() >= 1) {
        auto node_id = args[0];
        std::vector<NodeInfoPtr> nodes;

        std::unique_lock<std::mutex> lock2(all_nodes_mutex_);
        for (auto& kv : all_nodes_map_) {
            if (HexSubstr(kv.first) == node_id) {
