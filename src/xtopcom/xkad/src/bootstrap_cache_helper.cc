...
bool BootstrapCacheHelper::Start(
    ...
    bootstrap_cache_ptr_ = std::make_shared<BootstrapCache>(kad_key->GetServiceType());
    //LoadBootstrapCache();
...
uint64_t BootstrapCacheHelper::GetRandomCacheServiceType() {
    ...
    uint64_t this_time_service_type = top::kInvalidType;
    {
        ...
        uint32_t rand_idx = RandomUint64() % cache_service_types_.size();
    ...
    if (this_time_service_type == top::kInvalidType) {
        TOP_WARN("BootstrapCacheHelper:: this time service_type choose invalid");
        return top::kInvalidType;
    }
...
void BootstrapCacheHelper::CacheServicePublicNodes(uint64_t service_type) {
    ...
    }
    {
        ...
        if (cache_service_types_.find(service_type) == cache_service_types_.end()) {
            TOP_WARN("BootstrapCacheHelper:: service_type[%llu] not set yet", service_type);
            return;
        }
    }
...
bool BootstrapCacheHelper::GetCacheServicePublicNodes(
    ...
    if (service_nodes.empty()) {
        TOP_WARN("BootstrapCacheHelper:: GetCacheServicePublicNodes failed for service_type[%llu]",
                service_type);
        return false;
    }
...
void BootstrapCacheHelper::RegisterBootstrapCacheCallback(
        on_bootstrap_cache_get_callback_t get_cache_callback,
        on_bootstrap_cache_set_callback_t set_cache_callback) {
    assert(bootstrap_cache_ptr_);
    bootstrap_cache_ptr_->RegisterBootstrapCacheCallback(get_cache_callback, set_cache_callback);
    LoadBootstrapCache();
}

void BootstrapCacheHelper::UnRegisterBootstrapCacheCallback() {
    assert(bootstrap_cache_ptr_);
    bootstrap_cache_ptr_->UnRegisterBootstrapCacheCallback();
}
