...
std::atomic<uint32_t> CallbackManager::msg_id_(RandomUint32());
...
CallbackManager::CallbackManager()
    ...
    timer_->Start(
            kTimeCheckoutPeriod,
            kTimeCheckoutPeriod,
            bind(&CallbackManager::TimeoutCheck,this));
}
...
void CallbackManager::Remove(uint32_t message_id) {
    std::unique_lock<std::mutex> lock(callback_map_mutex_);
    callback_map_.erase(message_id);
}
