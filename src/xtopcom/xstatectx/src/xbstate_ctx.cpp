...
int32_t xbstate_ctx_t::tep_token_withdraw(const std::string & token_name, evm_common::u256 sub_token) {
    ...
    if (token_name == top::to_string(common::xtoken_id_t::eth)) {
        auto * engine_ptr = evm::evm_engine();
        auto * executor_ptr = evm::evm_executor();
        if (engine_ptr != nullptr && executor_ptr != nullptr) {
            auto const & account_address = this->account_address();
            assert(!account_address.empty());
            std::error_code ec;
            auto const & eth_address = common::xeth_address_t::build_from(account_address, ec);
            if (ec) {
                xwarn("xbstate_ctx_t::tep_token_withdraw, invalid account %s", account_address.c_str());
                return xaccount_property_operate_fail;
            }

            if (!burn(engine_ptr, executor_ptr, eth_address.data(), eth_address.size(), sub_token.str().c_str())) {
                xwarn("xbstate_ctx_t::tep_token_withdraw failed");
                return xaccount_property_operate_fail;
            }
            return 0;
        }
...
int32_t xbstate_ctx_t::tep_token_deposit(const std::string & token_name, evm_common::u256 add_token) {
    ...
    if (token_name == top::to_string(common::xtoken_id_t::eth)) {
        ...
        if (engine_ptr != nullptr && executor_ptr != nullptr) {
            ...
            auto const & eth_address = common::xeth_address_t::build_from(account_address, ec);
            if (ec) {
                xwarn("xbstate_ctx_t::tep_token_deposit, invalid account %s", account_address.c_str());
                return xaccount_property_operate_fail;
            }

            if (!mint(engine_ptr, executor_ptr, eth_address.data(), eth_address.size(), add_token.str().c_str())) {
                xwarn("xbstate_ctx_t::tep_token_deposit failed");
                return xaccount_property_operate_fail;
            }
            return 0;
        }
