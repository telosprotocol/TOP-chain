...
xtop_message_filter_message_id::xtop_message_filter_message_id(observer_ptr<vnetwork::xvhost_face_t> const &,
                                                               observer_ptr<election::cache::xdata_accessor_face_t> const &) noexcept {
}

bool xtop_message_filter_message_id::filter(xvnetwork_message_t &, std::error_code &) const {
    return true;
}
    ...
    do {
        ...
        if (recver.logic_epoch().empty()) {
            if (recver.account_election_address().empty()) {
                ...
                vnetwork_message.receiver(common::xnode_address_t{ sender.group_address(), recver.account_election_address(), sender.logic_epoch() });
            }
        }
        assert(sender.logic_epoch().has_value() && recver.logic_epoch().has_value());
    } while (false);
...
bool xtop_message_filter_recver_is_validator::filter_sender_from_associated_auditor(xvnetwork_message_t & vnetwork_message, std::error_code & ec) const {
    ...
    auto associated_auditor_group = m_election_data_accessor->parent_group_element(recver.group_address(), recver.logic_epoch(), ec);
    if (ec) {
        xinfo("[vnetwork][message_filter] hash: %" PRIx64 ", network %" PRIu32 " node %s receives msg sent to %s from %s. ignored. error: %s",
              vnetwork_message.hash(),
              static_cast<std::uint32_t>(m_vhost->network_id().value()),
...
bool xtop_message_filter_recver_is_auditor::filter_sender_from_edge(xvnetwork_message_t & vnetwork_message, std::error_code & ec) const {
    ...
    // fix receiver if necessary.
    if (recver.logic_epoch().empty()) {
        auto const auditor_group = m_election_data_accessor->group_element(recver.group_address(), recver.logic_epoch(), ec);
        if (ec) {
            xinfo("[vnetwork][message_filter] hash: %" PRIx64 ", network %" PRIu32 " node %s receives msg sent to %s from %s. ignored. error: %s",
                  vnetwork_message.hash(),
                  static_cast<std::uint32_t>(m_vhost->network_id().value()),
...
bool xtop_message_filter_recver_is_auditor::filter_sender_from_associated_validator(xvnetwork_message_t & vnetwork_message, std::error_code & ec) const {
    ...
    auto auditor_group = m_election_data_accessor->group_element(recver.group_address(), recver.logic_epoch(), ec);
    if (ec) {
        xinfo("[vnetwork][message_filter] hash: %" PRIx64 ", network %" PRIu32 " node %s receives msg sent to %s from %s. ignored. error: %s",
              vnetwork_message.hash(),
              static_cast<std::uint32_t>(m_vhost->network_id().value()),
    ...
    if (recver.logic_epoch().has_value()) {
        ...
        if (auditor_group->logic_epoch() != sender_associated_group->logic_epoch()) {
            auditor_group = sender_associated_group;
        }
...
bool xtop_message_filter_recver_is_rec::filter_sender_from_rec(xvnetwork_message_t & vnetwork_message, std::error_code & ec) const {
    ...
    if (recver.logic_epoch().empty()) {
        if (recver.account_election_address().empty()) {
            ...
            vnetwork_message.receiver(common::xnode_address_t{ recver.group_address(), recver.account_election_address(), sender.logic_epoch() });
        }
    }
...
bool xtop_message_filter_recver_is_rec::filter_sender_from_non_rec(xvnetwork_message_t & vnetwork_message, std::error_code & ec) const {
    ...
    // fix receiver if necessary.
    if (recver.logic_epoch().empty()) {
        auto const rec_group = m_election_data_accessor->group_element(recver.group_address(), recver.logic_epoch(), ec);
        if (ec) {
            xinfo("[vnetwork][message_filter] hash: %" PRIx64 ", network %" PRIu32 " node %s receives msg sent to %s from %s. ignored. error: %s",
                  vnetwork_message.hash(),
                  static_cast<std::uint32_t>(m_vhost->network_id().value()),
...
bool xtop_message_filter_recver_is_zec::filter_sender_from_non_zec(xvnetwork_message_t & vnetwork_message, std::error_code & ec) const {
    ...
    // fix receiver if necessary.
    if (recver.logic_epoch().empty()) {
        auto const zec_group = m_election_data_accessor->group_element(recver.group_address(), recver.logic_epoch(), ec);
        if (ec) {
            xinfo("[vnetwork][message_filter] hash: %" PRIx64 ", network %" PRIu32 " node %s receives msg sent to %s from %s. ignored. error: %s",
                  vnetwork_message.hash(),
                  static_cast<std::uint32_t>(m_vhost->network_id().value()),
