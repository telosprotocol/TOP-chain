...
void xcrc32_t::add(const void* data, size_t numBytes)
{
...
void xcrc32_t::add(const std::string & text)
{
    if(text.empty() == false)
      add(text.data(),text.size());
}

std::string xcrc32_t::crc32_to_string(const uint32_t int_hash)
{
    ...
    hashBuffer[0] = dec2hex[ int_hash >> 28      ];
    hashBuffer[1] = dec2hex[(int_hash >> 24) & 15];
    hashBuffer[2] = dec2hex[(int_hash >> 20) & 15];
    hashBuffer[3] = dec2hex[(int_hash >> 16) & 15];
    hashBuffer[4] = dec2hex[(int_hash >> 12) & 15];
    hashBuffer[5] = dec2hex[(int_hash >>  8) & 15];
    hashBuffer[6] = dec2hex[(int_hash >>  4) & 15];
    hashBuffer[7] = dec2hex[ int_hash        & 15];
...
std::string xcrc32_t::get_hash_string()
{
    return crc32_to_string(m_hash);
}

uint32_t   xcrc32_t::get_hash_int()
{
    return m_hash;
}

uint32_t xcrc32_t::crc32(const void* data, size_t numBytes)
{
    if( (NULL == data) || (numBytes <= 0) )
        return 0;
    
    xcrc32_t obj;
    obj.add(data, numBytes);
    return obj.get_hash_int();
}

uint32_t xcrc32_t::crc32(const std::string& text)
{
    if(text.empty())
        return 0;
