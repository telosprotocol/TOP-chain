...
int xrpc_eth_query_manager::set_relay_block_result(const xobject_ptr_t<base::xvblock_t>& block, xJson::Value & js_rsp, int have_txs, int blocklist_type) {
    if (block == nullptr) {
        js_rsp["result"] = xJson::Value::null;
        return 1;
    }
...
void xrpc_eth_query_manager::topRelay_getBlockByHash(xJson::Value & js_req, xJson::Value & js_rsp, string & strResult, uint32_t & nErrorCode) {
    ...
    if (js_req.size() >= 2){
        ...
        if (js_req[2].isInt()) {
            block_list_type = js_req[2].asInt();
        }
    }
    ...
    set_relay_block_result(block, js_rsp, have_txs, block_list_type);
}
