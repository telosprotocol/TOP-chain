...
void xshard_rpc_handler::process_msg(const xrpc_msg_request_t & edge_msg) {
    ...
    case enum_xrpc_tx_type::enum_xrpc_tx_type: {
        ...
        if (xsuccess != m_txpool_service->request_transaction_consensus(tx_ptr, false)) {
            ...
            xwarn("[global_trace][shard_rpc][push unit_service] fail %s,%s", tx_hash.c_str(), tx_ptr->source_address().to_string().c_str());
        } else {
            xkinfo("[global_trace][shard_rpc][push unit_service] succ %s,%s", tx_hash.c_str(), tx_ptr->source_address().to_string().c_str());
        }
        break;
    }
    default: {
