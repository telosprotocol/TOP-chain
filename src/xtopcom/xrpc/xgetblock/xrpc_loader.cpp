...
xJson::Value xrpc_loader_t::parse_confirm_tx(const xtxindex_detail_ptr_t & sendindex, const xtxindex_detail_ptr_t & confirmindex) {
    xJson::Value jv;
    if (nullptr != confirmindex) {
...
xJson::Value xrpc_loader_t::load_and_parse_recv_tx(const std::string & raw_tx_hash, const xtxindex_detail_ptr_t & sendindex, bool & is_missing_index) {
    xJson::Value jv;
    if (sendindex->get_txaction().get_inner_table_flag()) {  // not need recvindex, create a mock recv json
        ...
        if (recvindex != nullptr) {
            ...
            is_missing_index = true;  // missing txindex
        }
    }
    return jv;
}

xJson::Value xrpc_loader_t::load_and_parse_confirm_tx(const std::string & raw_tx_hash, const xtxindex_detail_ptr_t & sendindex, const xJson::Value & recvjson, bool & is_missing_index) {
    xJson::Value jv;
    if (sendindex->get_txaction().get_not_need_confirm()) {
        jv = xrpc_loader_t::parse_confirm_tx(sendindex, nullptr);
    } else {
        ...
        if (confirmindex != nullptr) {
            jv = xrpc_loader_t::parse_confirm_tx(sendindex, confirmindex);
        } else {
            is_missing_index = true;  // missing txindex
        }
    }
    if (false == is_missing_index) {
        if (recvjson["exec_status"].asString() == "fail") {
            jv["exec_status"] = "fail";  // XTODO set confirmtx status same with recvtx for compatibility
        }
    }
    return jv;
}

std::string xrpc_loader_t::parse_tx_state(bool is_missing_index, const xJson::Value & sendjson, const xJson::Value & recvjson, const xJson::Value & confirmjson) {
    std::string txstate;
    if (is_missing_index) {
        txstate = "pending";
    } else {
        if (sendjson["exec_status"].asString() == "fail"
            || recvjson["exec_status"].asString() == "fail"
            || confirmjson["exec_status"].asString() == "fail") {
            txstate = "fail";
        } else {
            txstate = "success";
        }
