...
xJson::Value get_block_handle::parse_tx(const uint256_t & tx_hash, xtransaction_t * cons_tx_ptr, const std::string & rpc_version) {
    ...
    if (tx_store_ptr != nullptr && tx_store_ptr->get_raw_tx() != nullptr) {
        ...
        if (tx_ptr->get_target_addr() != black_hole_addr && (tx_ptr->get_source_addr() != tx_ptr->get_target_addr())) {
            cons[jk.m_send] = get_tx_exec_result(tx_store_ptr->get_send_addr(), tx_store_ptr->get_send_unit_height(), tx_ptr, recv_txinfo, rpc_version);
            ...
            cons[jk.m_recv] = get_tx_exec_result(tx_store_ptr->get_recv_addr(), tx_store_ptr->get_recv_unit_height(), tx_ptr, recv_txinfo, rpc_version);
        }
        cons[jk.m_confirm] = get_tx_exec_result(tx_store_ptr->get_send_addr(), tx_store_ptr->get_confirm_unit_height(), tx_ptr, recv_txinfo, rpc_version);
