...
xevm_server::xevm_server(shared_ptr<xrpc_edge_vhost> edge_vhost,
    ...
    if (m_rpc_service == nullptr) {
        m_rpc_service = top::make_unique<xevm_rpc_service<xedge_evm_http_method>>(edge_vhost, xip2, archive_flag, block_store, txstore, elect_main, election_cache_data_accessor);
    } else {
        m_rpc_service->reset_edge_method_mgr(edge_vhost, xip2);
    }
}

void xevm_server::start(uint16_t nPort, uint32_t nThreadNum) {
    if (m_is_running) {
    ...
    m_server.resource["/"]["POST"] = std::bind(&xevm_server::start_service, this, std::placeholders::_1, std::placeholders::_2);
