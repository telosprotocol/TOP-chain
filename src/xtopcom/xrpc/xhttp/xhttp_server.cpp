...
xhttp_server::xhttp_server(shared_ptr<xrpc_edge_vhost> edge_vhost,
    ...
    if (m_rpc_service == nullptr) {
        m_rpc_service = top::make_unique<xrpc_service<xedge_http_method>>(edge_vhost, xip2, archive_flag, store, block_store, elect_main, election_cache_data_accessor);
    } else {
...
void xhttp_server::start(uint16_t nPort, uint32_t nThreadNum) {
    ...
    if (m_enable_ratelimit) {
        ...
        m_ratelimit->RegistRequestOut([](RatelimitData * data) {
