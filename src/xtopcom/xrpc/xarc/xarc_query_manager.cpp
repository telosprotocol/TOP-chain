...
void xarc_query_manager::get_property(xjson_proc_t & json_proc) {
    ...
    if (xverifier::xtx_utl::address_is_valid(account) != xverifier::xverifier_error::xverifier_success) {
        xwarn("xtx_verifier address_verify address invalid, account:%s", account.c_str());
        return;
    }
...
void xarc_query_manager::getBlock(xjson_proc_t & json_proc) {
    ...
    if (xverifier::xtx_utl::address_is_valid(owner) != xverifier::xverifier_error::xverifier_success) {
        xwarn("xtx_verifier address_verify address invalid, account:%s", owner.c_str());
        return;
    }
...
void xarc_query_manager::listVoteUsed(xjson_proc_t & json_proc) {
    ...
    // add top address check
    std::string account = json_proc.m_request_json["params"]["account_addr"].asString();
    if (xverifier::xtx_utl::address_is_valid(account) != xverifier::xverifier_error::xverifier_success) {
        xwarn("xtx_verifier address_verify address invalid, account:%s", account.c_str());
        return;
    }
