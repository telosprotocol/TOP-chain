...
void xrpc_query_manager::getIssuanceDetail(xJson::Value & js_req, xJson::Value & js_rsp, string & strResult, uint32_t & nErrorCode) {
    ...
    for (auto const & node_reward : issue_detail.m_node_rewards) {
        if (version == RPC_VERSION_V3) {
            ...
            node_reward_json["edge_reward"] = std::stod(ss.str(), &size);
            ss.clear();
            ...
            node_reward_json["archive_reward"] = std::stod(ss.str(), &size);
            ss.clear();
            ...
            node_reward_json["validator_reward"] = std::stod(ss.str(), &size);
            ss.clear();
            ...
            node_reward_json["auditor_reward"] = std::stod(ss.str(), &size);
            ss.clear();
            ...
            node_reward_json["voter_reward"] = std::stod(ss.str(), &size);
            ss.clear();
            ...
            node_reward_json["self_reward"] = std::stod(ss.str(), &size);
    ...
    for (auto const & property : property_names) {
        ...
        if (version == RPC_VERSION_V3) {
            ...
            for (auto i : auditor.getMemberNames()) {
                auditor[i][0]["account_addr"] = i;
                array1.append(auditor[i][0]);
            }
            tmp["auditor"] = array1;
            xJson::Value array2;
            xJson::Value validator = j["validator"];
            for (auto i : validator.getMemberNames()) {
                validator[i][0]["account_addr"] = i;
                array2.append(validator[i][0]);
            }
