...
void xrpc_query_manager::getChainId(xJson::Value & js_req, xJson::Value & js_rsp, std::string & strResult, uint32_t & nErrorCode) {
    ...
    getCrossReceiptIds(js_req, js_rsp, strResult, nErrorCode);  // TODO(jimmy) test
...
void xrpc_query_manager::getCrossReceiptIds(xJson::Value & js_req, xJson::Value & js_rsp, std::string & strResult, uint32_t & nErrorCode) {  
    ...
    for (auto & info : unfinish_infos) {
        xJson::Value v;
        // v["source_height"]      = static_cast<unsigned long long>(info.source_height);
        // v["target_height"]      = static_cast<unsigned long long>(info.target_height);
        v["source_table_id"]    = info.source_id;
        v["target_table_id"]    = info.target_id;
        v["unrecv_num"]         = info.unrecv_num;
        v["unconfirm_num"]      = info.unconfirm_num;
        jv_unfinish_info.append(v);
    }

    xJson::Value jv_value;
    jv_value["tables_height"] = jv_tables_height;
    jv_value["unfinish_infos"] = jv_unfinish_info;
    js_rsp["value"] = jv_value;
}
