...
void xedge_method_base<T>::sendTransactionV2_method(const std::string& raw_tx, xjson_proc_t & json_proc, const std::string & ip) {
    ...
    xChainSDK::protocol tran_protocol;
    xjson_proc_t json_proc_temp;
    tran_protocol.encode_transaction_params(json_proc_temp.m_request_json, tx.get());
    xfilter_manager filter_manager;
    filter_manager.sendTransaction_filter(json_proc_temp);
