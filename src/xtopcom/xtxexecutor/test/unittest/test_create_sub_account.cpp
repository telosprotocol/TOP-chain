...
class create_sub_account : public testing::Test {
 protected:
    void SetUp() override {
...
TEST_F(create_sub_account, sub_account) {
    ...
    xtransaction_create_sub_account sub_account(m_account_context.get(), m_trans);
    ...
    stream_s << (uint64_t)XGET_CONFIG(min_account_deposit);
    ...
    m_trans->get_target_action().set_action_type(enum_xaction_type::xaction_type_asset_out);
    ...
    sub_account.parse();
    sub_account.check();
    int32_t ret = sub_account.source_action_exec();
    EXPECT_EQ(0, ret);
