...
TEST_F(test_vote, pledge_vote_60days) {
    m_source_context->get_blockchain()->set_balance(ASSET_TOP(10000));
    data::xaction_pledge_token_vote action;
    ...
    ret = pledge_token_vote.target_action_exec();
    EXPECT_EQ(0, ret);
    double factor = config::get_top_vote_rate(60);
    EXPECT_EQ(static_cast<uint64_t>(TOP_UNIT * vote_num / factor), m_source_context->get_vote_balance_change());
    std::cout << std::setprecision(6) << std::fixed << "factor " << factor
              << " vote num: " << vote_num
