...
TEST_F(test_pending_account, sigle_send_tx) {
    ...
    ASSERT_EQ(0, ret);
    auto tx_tmp = pending_accounts.find(tx->get_transaction()->get_source_addr(), tx->get_tx_hash_256());
    ...
    tx_tmp = pending_accounts.find(tx->get_transaction()->get_source_addr(), tx->get_tx_hash_256());
...
TEST_F(test_pending_account, sigle_account_multi_send_tx) {
    ...
    uint32_t i = 0;
    for (; i <= 1; i++) {
        tx_tmp = pending_accounts.find(txs[i]->get_transaction()->get_source_addr(), txs[i]->get_tx_hash_256());
        ASSERT_EQ(tx_tmp, nullptr);
    }
