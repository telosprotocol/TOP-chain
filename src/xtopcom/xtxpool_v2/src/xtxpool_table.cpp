        ...
        if (tx->get_tx()->is_recv_tx()) {
            ...
            if (ret2) {
                if (account_index.get_latest_unit_hash().empty()) {  // TODO(jimmy) only for old version accountindex delete future
                    ...
                    data::xblocktool_t::check_lacking_unit_and_try_sync(_account_vaddress, account_index, latest_connect_height, m_para->get_vblockstore(), "txpool");
                }
            } else {
...
int32_t xtxpool_table_t::verify_send_tx(const xcons_transaction_ptr_t & tx, bool is_first_time_push_tx) const {
    ...
    if (!data::is_sys_contract_address(common::xaccount_address_t{raw_tx->get_source_addr()}) && raw_tx->get_target_addr() == sys_contract_rec_standby_pool_addr) {
