...
bool xtxpool_table_t::get_account_basic_info(const std::string & account, xaccount_basic_info_t & account_index_info, uint64_t & nonce) {
    ...
    xtxpool_dbg("xtxpool_table_t::get_account_basic_info nathan_nonce_test account:%s,nonce:%llu", account.c_str(), nonce);
    ...
    // if (latest_connect_height == 0) {
    //     // TODO(jimmy) delete future
    //     xtxpool_info("jimmy xtxpool_table_t::get_account_basic_info account=%s,index=%s,latest_connect_height is zero", _account_vaddress.get_account().c_str(), account_index.dump().c_str());
    //     nonce = 0;
    //     return true;
    // }

    // base::xauto_ptr<base::xvbstate_t> account_bstate =
    //     base::xvchain_t::instance().get_xstatestore()->get_blkstate_store()->get_block_state(_account_vaddress,latest_connect_height,latest_connected_hash, metrics::statestore_access_from_txpool_get_accountstate);
...
const std::vector<xcons_transaction_ptr_t> xtxpool_table_t::get_resend_txs(uint64_t now) {
    ...
    for (auto & resend_id_height : sender_resend_vec) {
        auto receipt = build_receipt(resend_id_height.table_sid, resend_id_height.receipt_id, resend_id_height.height, enum_transaction_subtype_send);
        if (receipt != nullptr) {
