...
void xtxpool_t::refresh_black_addr_txs() {
    std::vector<std::string> new_addrs = xverifier::xtop_blacklist_utl::instance().refresh_and_get_new_addrs();
    if (new_addrs.empty()) {
        return;
    }
    xinfo("xtxpool_t::refresh_black_addr_txs begin clear black addrs. count=%zu", new_addrs.size());
    ...
    for (auto & table_index : all_table_indexs) {
        for (uint16_t i = 0; i < table_index.second; i++) {
            auto table = get_txpool_table(table_index.first, i);
            if (table != nullptr) {
                for (auto & addr : new_addrs) {
                    table->clear_black_address_txs(addr);
                }                
            }
        }
    }
    xinfo("xtxpool_t::refresh_black_addr_txs finish clear black addrs. count=%zu", new_addrs.size());
}
