...
int32_t xunconfirmed_account_t::update(xblock_t * latest_committed_block, const xtable_state_cache_t & table_state_cache) {
    ...
    for (uint64_t cur_height = latest_height; cur_height > m_highest_height; cur_height--) {
        ...
        if (cur_height == latest_height) {
            ...
            auto _commit_block = m_para->get_vblockstore()->load_block_object(account_addr, cur_height, last_block_hash, false, metrics::blockstore_access_from_txpool_refresh_table);
        ...
        if (!unconfirm_sendtx_actions.empty()) {
            auto _cert_block = m_para->get_vblockstore()->load_block_object(account_addr, cur_height + 2, 0, false, metrics::blockstore_access_from_txpool_refresh_table);  // only need cert mini-block
...
void xunconfirmed_tx_queue_t::recover(const xtable_state_cache_t & table_state_cache, const xtablestate_ptr_t & tablestate) {
    ...
    for (auto & account : accounts) {
        ...
        base::xauto_ptr<base::xvblock_t> unitblock = m_para->get_vblockstore()->load_block_object(_vaccount, account_index.get_latest_unit_height(), account_index.get_latest_unit_viewid(), false, metrics::blockstore_access_from_txpool_refresh_table);
