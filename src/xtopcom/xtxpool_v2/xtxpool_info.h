...
public:
    ...
    ~xtxpool_table_info_t() {
        ...
        XMETRICS_GAUGE(metrics::txpool_unconfirm_tx_cur, -m_counter.get_unconfirm_tx_count());
    ...
    void set_unconfirm_tx_count(int32_t count) {
        ...
        for (auto & shard : m_shards) {
            xtxpool_dbg("set_unconfirm_tx_count shard(%p) table:%s old unconfirm count:%d inc count:%d", shard, get_address().c_str(), shard->get_unconfirm_tx_count(), count - old_count);
