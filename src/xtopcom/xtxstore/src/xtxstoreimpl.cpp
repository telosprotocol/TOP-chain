...
std::vector<base::xvblock_ptr_t> xtxstoreimpl::load_block_objects(const std::string & tx_hash, const base::enum_transaction_subtype type) {
    ...
    for (auto & block : blocks) {
        if (false == top::base::xvchain_t::instance().get_xblockstore()->load_block_body(base::xvaccount_t(block->get_account()), block.get(), base::enum_xvblock_body_type_all)) {
            xerror("xtxstoreimpl::load_block_objects for txhash fail-load block body. block=%s", block->dump().c_str());
            return {};
        }
    }
    return blocks;
