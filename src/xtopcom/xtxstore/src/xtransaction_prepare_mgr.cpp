...
int xtransaction_prepare_mgr::update_prepare_cache(const data::xblock_ptr_t bp) {
    ...
    for(auto action : input_actions) {
        ...
        if (cache_data.rpc_version == data::RPC_VERSION_V2) {
            jv["account"] = tx_ptr->get_source_addr();
        }
        ...
        const chain_info::xtx_exec_json_key jk(cache_data.rpc_version);
        if (_subtype == base::enum_transaction_subtype_self) {
            ...
            ji[jk.m_send]["tx_fee"] = static_cast<xJson::UInt64>(beacon_tx_fee);
